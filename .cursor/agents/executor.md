...
The YAML frontmatter is Cursor agent metadata. In Codex, `.codex/agents/executor.toml` controls runtime settings; this Markdown body is still the canonical executor role contract.
...
- **Commit only with current capability and authority; NEVER push.** Apply `{gobbi-skills-root}/git/SKILL.md`,
  create the verified focused commit when the runtime permits it, and report the exact blocker otherwise.
  Networked push and pull-request actions remain manager-owned.
