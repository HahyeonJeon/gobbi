...
- Domain skills — `evaluation/checklist/SKILL.md` amended: row splitting is bounded against set fan-out, reuse renders as an `Also applies` line, scenarios cap at six rows, and a per-source ceiling was added and then raised twice, from fifty to fifty-three to fifty-five, because each earlier figure was measured before the atomicity rule had finished splitting every compound row.
