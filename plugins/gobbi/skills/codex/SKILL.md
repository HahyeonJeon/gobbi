...
Every command, flag, and status below was verified against Codex CLI 0.147.0 on Linux. Codex changes quickly,
...
Canonical skills and role contracts have exactly one owner. Local discovery stays complete, while package
skills are a filtered generated view: every included path must be byte-equal to canonical and every registered
repository-only root must remain absent from the package and installed cache.
...
Exit statuses observed on 0.147.0: a completed run exits `0`; an unknown flag or an invalid value for a known
...
| Installed plugin skill | `gobbi:{skill}` | A filtered distributable skill; registered repository-only roots are absent |
...
Verified against 0.147.0, the CLI does not load a repository's `.codex/config.toml` — a probe repository whose
...
package contains both plugin manifests, a filtered generated `skills/` directory, and a complete generated
`agents/` directory. Every included path must be byte-equal to its canonical owner, while an explicitly
registered repository-only skill root must be absent. Regenerate both components with
`bash scripts/sync-plugin-package.sh --materialize-package`. A valid current install delivers both manifests
and both component directories. The generator validates each registered repository-only owner before any
mutation. A symlinked component root or nested package path is invalid; a missing included path, leaked
repository-only root, or incomplete installed component fails. Repository-local `.agents/skills` remains
complete and is not filtered.
...
| An included installed plugin path is missing | Treat it as a failure; inspect the named package path for a symlink, then regenerate with `bash scripts/sync-plugin-package.sh --materialize-package` |
| An installed cache exposes a registered repository-only root | Treat it as a projection failure; preserve the cache and inspect the exact package filter and stale paths |
