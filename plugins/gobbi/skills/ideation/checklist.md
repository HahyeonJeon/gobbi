...
#### An accepted change leaves related design surfaces inconsistent

- [ ] An accepted change is absent from an affected requirement, topic, decision, design surface, risk, or deferral.
- [ ] New evidence invalidates a premise without reopening the earliest dependent decision.
- [ ] A superseded decision remains presented as current.
- [ ] A changed scope boundary lacks traceable authority.
- [ ] A correction conflicts with another part that still appears current.

## Design and Development Lifecycle

### Result Integrity

#### The indexed result has ambiguous authority, incomplete membership, or an invalid frozen identity

- [ ] The authoritative result locator or membership boundary is ambiguous.
- [ ] A listed authoritative member is missing or unreadable.
- [ ] An output inside the result boundary is absent from the declared membership.
- [ ] A member is reached more than once through the indexes.
- [ ] The indexes do not define one complete reading order.
- [ ] The root index, an authoritative member, and a subordinate snapshot express conflicting authority.
- [ ] A listed member falls outside the accepted output boundary.
- [ ] The same frozen identity refers to different membership, order, or content.

### Problem and Requirements

#### The design is founded on an unsupported problem or unusable outcomes

- [ ] The stated problem conflicts with the supported current behavior or evidence.
- [ ] Inspected facts, attributed reports, hypotheses, and uncertainty are treated as equivalent evidence.
- [ ] The affected actors or material impact cannot be identified.
- [ ] The desired outcome is not observable.
...
#### Risks, validation, or deferrals hide uncertainty instead of resolving it safely

- [ ] A material risk or load-bearing assumption is absent.
- [ ] Planned validation is presented as completed evidence.
- [ ] A proposed validation method cannot answer its stated question.
- [ ] Validation signals do not distinguish observable pass and fail states.
...
#### Handoff or revision makes the design unsafe to consume or reuse

- [ ] A consumer cannot identify the exact current result and its authoritative membership.
- [ ] Authoritative, subordinate, and superseded material cannot be distinguished.
- [ ] The handoff omits a material risk, deferral, validation need, reopen condition, or recovery point.
- [ ] A revision changes accepted meaning without traceable authority.
- [ ] A replacement strands downstream work on a superseded or contradictory design.
