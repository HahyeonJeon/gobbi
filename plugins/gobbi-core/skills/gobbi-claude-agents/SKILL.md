...
**Frontmatter** — Required fields: `name`, `description`, `tools` (scoped to what the agent actually needs). Include `model` for non-evaluator agents (currently `opus`). Evaluator agents omit `model` to use the default. The `description` field is critical — it answers "when should the orchestrator send work here?" If two agents' descriptions match the same task, boundaries need sharpening.
...
- **Model selection** — What model should this agent use? Non-evaluator agents typically use opus. Evaluator agents omit model to use the default.
