...
created-at: 2026-05-23T18:30:00Z
iter: 2
supersedes: rawdata/draft-iter1.md
revision-of-record: |
  iter1 closed REVISE both systems. iter2 addresses 7 findings spanning convergent
  high (R1 / COD-STRUCT-002 concurrent lost-update race; P1 / C1 invented trailer
  segment) and Codex-only high (COD-PROJ-001 branch naming, COD-STRUCT-001
  resolver, COD-STRUCT-003 correlation key) plus the COD-CONS D-3-4-vs-T3-I-3
  tension. Scope Contract is preserved; design adds D-3-5 (serialization),
  qualifies D-1 / D-3 / D-3-1 / D-3-3 / D-3-4, and trims P2/O1 by citing the
  official Claude Code hooks doc verbatim.
---

# Ideation Draft — iter2 (canonical)

Loop: Ideation. Session: `2026-05-23-1b26cf20-677b-498c-8c1b-7d7e971597ac`. Feature: `session-foundations-bundle-b`. iter2 carries the iter1 Scope Contract / Framed Problem / Research Insights / Scenarios forward verbatim and updates Implementation Checklist + Design + Decisions Log to address every iter1 High finding.

Source records: Sub-step A findings (`rawdata/sub-step-a-findings-iter1.md`), Sub-step C findings (`rawdata/sub-step-c-findings-iter1.md`), Sub-step D design (`rawdata/sub-step-d-design-iter1.md`), iter1 evaluations (`evaluation/iter1/{claude,codex}/*.md`). The 12 staged external reference files at `staging/references/` remain valid; one additional verification (official Claude Code hooks doc, fetched 2026-05-23) is recorded inline in this draft's Decisions Log.

---

## Scope Contract

(Carried forward verbatim from iter1 — no Scope Contract decision was reopened in iter2. The iter1 user locks on T1 worktree-first uniform, T2 deferral, T3 mechanism (c) hook + reconstructor, dual registration, per-iteration commit cadence, and direct mode opt-out all hold.)
...
- **T3 dual registration**: PostToolUse + PostToolUseFailure, single script (CP-D-1 Option Recommended). Both events confirmed as officially supported shell-command hooks per `https://code.claude.com/docs/en/hooks` (fetched 2026-05-23; see Decisions Log iter2 fix-decision F-3). Template `status` field deferred to backlog.
...
(Carried forward verbatim from iter1 — both T1 and T3 framings hold. The iter2 changes target Implementation Checklist + Design, not the framing.)
...
(Carried forward verbatim from iter1. The 12 staged references at `staging/references/` remain unchanged. One verbatim verification added inline below for finding 3 / P2 / O1.)
...
- **T3-E-5 (NEW iter2)** — `PostToolUseFailure` IS officially documented as a shell-command-supported hook event at `https://code.claude.com/docs/en/hooks` (fetched 2026-05-23 via WebFetch — see Decisions Log iter2 fix-decision F-3 for the verbatim takeaway). Why: closes iter1 P2/O1 (the assumption that `PostToolUseFailure` was only community-attested via `claudefa.st`). D-3-3 dual registration is retained with the official-doc citation now anchoring it.
...
(Carried forward from iter1 with one addition: G-3 cross-task end-to-end scenario surfaced by iter1 O2; one E-5 concurrent-hook scenario surfaced by iter1 R1; one F-4 partial-promotion-failure scenario surfaced by iter1 R2.)

### T1 scenarios

**Golden**

- **G-1** — Feature session, worktree-first bootstrap, in-session `generate-now` skill ships in PR. The user types `/gobbi`; Configuration Step 1 advances; new row 5.5 creates the worktree (branch `session/{date}-{ssid-short}` per D-1 finding 4 lock); row 6 stamps `git.branch` + `git.worktreePath` non-null; all subsequent agents receive worktree-absolute `cwd`. Preparation `generate-now` writes skill body + symlinks to worktree paths, then `git add` + `git commit` lands them on the worktree branch with the canonical `AI-Provenance-Record: gobbi://session/{session-id}/task/{task-id}` trailer (per D-3 finding 2 lock). PR diff is complete; merge ships intact. Anchors: T1-I-1, T1-I-2, T1-I-3, T1-E-1.
...
- **E-2** — Non-feature (investigation / doc-only / mistake-promotion / refactor) session under the uniform lock. Worktree on `session/{date}-{ssid-short}`-prefixed branch (per D-1 finding 4 lock); the only commit may be the session-memory commit (per D-4). User explicitly accepted this uniformity in Scope Contract (Sub-step A CP-1.3-γ Option A). Anchors: T1-I-2, T1-E-2 (community caveat), Sub-step A forcing-question 5 counterfactual.
...
- **G-3 (NEW iter2 — from iter1 O2)** — Joint end-to-end validation in the first post-merge session: `jq '.git.worktreePath' session.json` returns non-null AND `jq '.agents | length' session.json` returns ≥ 2 (manager seed + the first leader / executor spawn the next session performs). Witness: a single smoke-test command run by the user / manager at the first post-merge session's Memorization phase. Anchors: D-1, D-3-3, iter1 O2 finding.

---

## Implementation Checklist

Per-task, anchored items — each cites the design decision and source insight. Detailed mechanism deferred to Execution. iter2 additions are flagged inline.

### T1 implementation checklist

- **T1-I-T1.a** — Edit `.claude/skills/orchestration/SKILL.md` § Step 1 — Workflow Configuration table: insert new row 5.5 ("Create worktree (P2 wrapper) and stamp `git.worktreePath` for use by row 6") between current row 5 (state.json) and current row 6 (session.json stamp); idempotent guard. Branch name at creation is `session/{date}-{ssid-short}` (per D-1 finding 4 lock; rename to feature-scoped name is a follow-up only, not in scope). Rewrite row 6's existing "leave null until git creates the worktree" text. Anchors: T1-I-2, T1-E-1, D-1.
...
- **T1-I-T1.h** — THIS-session migration note: T1's docs land on develop via the existing direct-or-worktree-pr Execution flow for this session. Add a one-line Wrap-up note flagging this session as the migration boundary. Concrete smoke test gate (per iter1 P4): immediately after the merge, run `jq '.git.worktreePath' .gobbi/projects/gobbi/sessions/<latest>/session.json` on the next session and verify non-null. Anchors: Sub-step D § Implementation checklist item 10, iter1 P4.
- **T1-I-T1.i (NEW iter2 — from iter1 C2)** — Edit `.claude/skills/delegation/SKILL.md`: grep for any hardcoded "main-tree absolute path" boilerplate in the Load Directives Block or delegation-prompt templates; if found, qualify with the same rule as D-2 (use `session.json.git.worktreePath` when set). Verify by `grep -n 'main tree absolute' .claude/skills/delegation/SKILL.md` returning ≤ 1 match, qualified. Anchors: D-2 validation method, iter1 C2.
- **T1-I-T1.j (NEW iter2 — from iter1 R2 / F-4)** — Add explicit rollback semantics to the `preparation/SKILL.md` narrow-exception (T1-I-T1.d) text: if `git commit` fails after the file copy, the manager MUST `git -C "$worktreePath" rm` the copied file before surfacing to user via AskUserQuestion. Anchors: F-4, iter1 R2.

### T3 implementation checklist

- **T3-I-T3.a (UPDATED iter2 — finding 1)** — Create `.claude/hooks/post-tool-use-agents.sh` (new file): bash + jq strict mode; shape cloned from `session-start.sh`; resolves session.json via D-3-3 algorithm; acquires `flock -x <session.json>` per D-3-5 BEFORE read; two-tier defensive `jq`; upsert agents[] by `id` using D-3-6 correlation key; atomic write; releases lock. Anchors: T3-I-4, T3-E-1, T3-E-2, D-3-1, D-3-5, D-3-6.
- **T3-I-T3.b (UPDATED iter2 — finding 1)** — Create `.claude/scripts/reconstruct-agents.sh` (new file): bash + jq; takes session-dir path arg; walks transcript JSONL; acquires `flock -x <session.json>` per D-3-5 BEFORE read; verify-and-fix upsert; orphan-report only (no delete); idempotent; releases lock. Anchors: T3-I-2, T3-DQ-2, D-3-2, D-3-5.
- **T3-I-T3.c (UPDATED iter2 — finding 3)** — Edit `.claude/settings.json`: add two `hooks` blocks — `PostToolUse` matcher `"Task"` → `.claude/hooks/post-tool-use-agents.sh`; `PostToolUseFailure` matcher `"Task"` → same script (single script handles both via `hook_event_name` branching). `PostToolUseFailure` is officially supported per `https://code.claude.com/docs/en/hooks` (fetched 2026-05-23). Anchors: T3-I-5, T3-E-4, T3-E-5, D-3-3.
...
- **T3-I-T3.g (NEW iter2 — finding 1 design)** — Document the serialization primitive (`flock -x` on `session.json`) in `delegation/SKILL.md`'s structured-headers sub-section AND in the hook's leading comment block. Specify the lock file is `session.json` itself (Linux/macOS `flock(2)` operates on the file's open descriptor; the script opens the file with `exec {fd}>>"$session_json"` and runs `flock -x "$fd"`). Lock is RELEASED automatically when the script's process exits (per flock(2) semantics). Anchors: D-3-5, iter1 R1, iter1 COD-STRUCT-002.

### Process / docs

- **CL-1 (NEW iter2 — from iter1 COD-CONS-001 / COD-AESTH-001 / COD-USAGE-001)** — Add a one-paragraph reconciliation note in the iter2 Wrap-up (and in `gobbi/SKILL.md`'s Memory Access Matrix as a follow-up note) explaining the `.claude/skills/` vs `.agents/skills/` vs `.gobbi/projects/gobbi/skills/` path surfaces. Claude-runtime-facing files live under `.claude/skills/`; Codex-runtime + plugin-facing skills live under `.gobbi/projects/gobbi/skills/` with `.agents/skills/{slug}` symlinks pointing into them. T1 + T3's implementation checklist intentionally targets the `.claude/skills/` surface because every edit is a Claude-runtime skill-doc change (workflow / orchestration / git / preparation / delegation are Claude-runtime skills, not plugin-mirrored Codex skills). NOTE: this is a clarifying doc-sync note, not a re-scoping — the Scope Contract above remains accurate; iter2 just makes the path-vocabulary rationale explicit.

---

## Design

Directional design decisions per Sub-step D, organized by task. Each decision states the chosen direction, rationale anchored to an insight, trade-off considered, and validation method. Detailed mechanism (function bodies, exact regexes, file-level structure) is deferred to Execution. iter2 changes are flagged inline.

### T1 design decisions

**D-1 (UPDATED iter2 — finding 4) — Configuration Step 1 row order: insert worktree creation as row 5.5 (between state.json init and session.json stamp); idempotent (skip if `session.json.git.worktreePath` non-null on resume). Branch name at creation is `session/{date}-{ssid-short}` — a session-id-based form that does not require knowing the feature / task slug at Configuration time.**

- *Rationale.* Row 6 needs `git.branch` + `git.worktreePath` non-null; worktree creation must precede row 6. Inserting as row 5.5 (rather than promoting to row 5) preserves the existing semantic where `state.json` is initialized first. Idempotency handles resume + `/clear` + `/compact` (SessionStart matcher fires on all four per `.claude/settings.json:30-40`). **Finding 4 lock**: Codex evaluator surfaced COD-PROJ-001 — the iter1 row 5.5 design did not specify the branch name and `git/SKILL.md:159` requires a concrete `<branch-name>`. Three options were available: (a) defer worktree creation to Sub-step B end, (b) placeholder + rename, (c) session-id-based branch name not needing task slug. Picked **(c)** because: it preserves the Scope-Contract-locked row 5.5 placement (option (a) would move worktree creation out of Configuration, reopening the user-locked row order); it avoids the rename complexity of option (b); and the branch is short-lived (one session → squash-merge → delete) so the "less meaningful name" cost is bounded to the lifespan of the worktree.
- *Anchored insights.* T1-I-2, T1-E-1, T1-DQ-2; iter1 COD-PROJ-001.
- *Trade-off considered.* (a) Defer to Sub-step B end — rejected: reopens row 5.5 Scope Contract lock. (b) Placeholder + rename — rejected: rename step adds complexity (`git branch -m` + worktree directory rename + `session.json.git.branch` re-stamp). (c) Session-id-based — picked.
- *Validation.* Future-session smoke test on next `/gobbi` → `jq '.git.branch'` matches `session/[0-9]{4}-[0-9]{2}-[0-9]{2}-[a-f0-9]{8}`; Evaluator Consistency check on table numerical order; manual migration witness this session.
...
**D-3 (UPDATED iter2 — finding 2) — NEW absorbed (commit-on-branch for promote-now): `git -C "$worktreePath" add` + `git -C "$worktreePath" commit` runs as closing step of Preparation EXIT promote-now. Subject `chore(skills): promote {slug} generated by preparation iter{n}`. Required `AI-Provenance-Record: gobbi://session/{session-id}/task/{task-id}` trailer per `git/conventions.md:118` canonical form, with `{task-id}` = `preparation-promote-now-iter{n}` (the workflow-generated task slug for the promotion). Partial-failure rollback: if `git commit` fails post-copy, manager `git -C "$worktreePath" rm` the copied file and AskUserQuestion before re-attempt (per T1-I-T1.j / F-4).**

- *Rationale.* T1-I-3 — the narrow exception is already a sole-writer violation; commit-on-branch is the structural completion. `-C "$worktreePath"` is explicit about which tree; subject grammar locked by `git/conventions.md`. **Finding 2 lock**: iter1 invented a `loop/preparation/promote-now` segment that contradicts the only `AI-Provenance-Record:` row at `git/conventions.md:118` which mandates `task/{task-id}`. Freshly re-verified iter2: `git/conventions.md:118` reads "`AI-Provenance-Record: gobbi://session/{session-id}/task/{task-id}`" with example "`AI-Provenance-Record: gobbi://session/2026-05-20-abc123/task/03-add-cache-layer`". The corrected form uses `task/` not `loop/`; the task slug is the workflow-generated promotion identifier. Rollback semantics added per iter1 R2 to close F-4.
- *Anchored insights.* T1-I-3, T1-E-2, `git/conventions.md:118` (freshly verified iter2), T1-DQ-3, iter1 P1/C1, iter1 R2.
...
(UPDATED iter2 — finding 1: hook authoring now incorporates the D-3-5 serialization primitive as a mandatory step before read.)
...
(UPDATED iter2 — finding 1: reconstructor also acquires `flock -x` per D-3-5 before its read-modify-write cycle.)
...
**D-3-3 (UPDATED iter2 — finding 3) — Hook scope: register both `PostToolUse` + `PostToolUseFailure` with matcher `"Task"`; single script handles both (branches on `hook_event_name` / `status`). Failed-spawn entries get `status: "failed"` and synthetic `id` (= `tool_use_id`) when `agentId` is null.**

Session-dir resolver algorithm (per D-3-3 below, refined by finding 5).

- *Rationale.* T3-DQ-3 + E-1 — failed spawns are part of the audit trail. **`PostToolUseFailure` officially documented** as a shell-command-supported hook event at `https://code.claude.com/docs/en/hooks` (fetched 2026-05-23 via WebFetch — see Decisions Log iter2 fix-decision F-3). Single script keeps maintenance burden low.
- *Anchored insights.* T3-E-4, T3-E-5 (iter2 official-doc verification), T3-DQ-3, E-1, `codex-rescue-agent-fire-and-forget-without-result-capture.md`.
- *Trade-off considered.* PostToolUse only — rejected (loses failed-spawn audit; official-doc support eliminates the iter1 "only community-attested" objection). Two scripts — rejected (DRY violation).
...
**D-3-3-resolver (NEW iter2 — finding 5) — Session-dir resolver algorithm.**
...
1. **Project name lookup precedence**:
   ...
   - **(ii — fallback)** Enumerate `$cwd/.gobbi/projects/` and select the single directory if exactly one exists; if zero or multiple, exit non-zero with stderr `"session-dir resolver: cannot disambiguate project name (n=<count>)"` (reconstructor recovers).
...
- *Rationale.* iter1 COD-STRUCT-001 + COD-USAGE-002 + COD-RISK-002 flagged that the resolver was underspecified. The official hook contract (`session-start.sh:13-25` precedent) provides `session_id` + `cwd` only; project name and date prefix must be derived. Three options were available per the brief (project-name: (i) scan `.gobbi/projects/`, (ii) read `project.json`, (iii) extract from `git remote get-url origin`). Picked **(ii) + (i) fallback** because: `project.json` is the canonical project pointer when it exists; the directory scan provides a robust fallback for projects that haven't yet stamped it. For the date prefix: picked **(b) sessions-scan** because it does not depend on clock-correctness (option (a) `date +%Y-%m-%d` can be wrong if session crosses midnight; option (c) reads `session.json` which is the file we're trying to find — circular).
- *Anchored insights.* T3-I-2, T3-I-5, `session-start.sh:13-25`, iter1 COD-STRUCT-001, iter1 COD-USAGE-002, iter1 COD-RISK-002.
- *Trade-off considered.* (i) directory scan first — rejected (loses the explicit canonical pointer when `project.json` exists). (iii) git-remote derivation — rejected (couples gobbi project name to git remote URL; many projects have aliased remotes; brittle).
- *Validation.* Single-script verifier with two fixtures: (a) `project.json` present → reads name from it; (b) `project.json` absent + single dir under `.gobbi/projects/` → uses the dir name. Negative case fixture: zero dirs → resolver exits non-zero with stderr message.

**D-3-4 (UPDATED iter2 — finding 6) — Metadata extraction: hybrid — `model` from `tool_input.model`; `step / phase / iter / sub-step` parsed from `tool_input.prompt` via canonical structured headers. Convention codified in `delegation/SKILL.md`. Regex `^Your (phase|iteration|sub-step|step): (.+)$`.**

Explicit clarification (per finding 6): the hook stdin payload has TWO sides:
...
**D-3-5 (NEW iter2 — finding 1) — Serialization primitive: POSIX `flock -x` on `session.json` for every read-modify-write cycle in both the hook and the reconstructor.**
...
|---|---|---|---|
| D-1 row 5.5 placement + branch naming | Future-session smoke test (`jq '.git.branch'` matches `session/[0-9]{4}-...`) + Consistency check | Next session post-merge + Execution-time evaluator | Once next session starts |
...
| D-3-3-resolver | 2-fixture single-script verifier (project.json present / absent) + negative-case fixture | Executor | Execution-time |
...
Chronological summary of every AskUserQuestion outcome during DISCUSSION, plus reference + backlog promotion log. iter2 fix-decisions appended in their own subsection at the end.
...
Each fix-decision below traces to one or more iter1 findings and cites the source of authority (file:line) used. The fix is locked into the corresponding Design / Checklist sections above; this subsection is the audit trail.
...
**F-2 — Finding 2 (P1 / C1 — invented `AI-Provenance-Record` trailer segment).** Replaced `gobbi://session/{session-id}/loop/preparation/promote-now` (invented iter1) with canonical `gobbi://session/{session-id}/task/{task-id}` where `{task-id}` = `preparation-promote-now-iter{n}` (workflow-generated task slug). Source of authority: `git/conventions.md:118` freshly verified iter2 — the row reads "`AI-Provenance-Record: gobbi://session/{session-id}/task/{task-id}`" with no `loop/` variant. Implementation impact: D-3 narrative + T1-I-T1.d both updated.

**F-3 — Finding 3 (P2 / O1 — `PostToolUseFailure` shell-hook unverified).** Verified via WebFetch of `https://code.claude.com/docs/en/hooks` on 2026-05-23. Result: `PostToolUseFailure` IS officially documented as a shell-command-supported hook event (one of 31 documented events; explicitly listed in the doc with decision-control behavior). Outcome (i) per brief: KEEP D-3-3 dual registration; ADD official-doc citation. Implementation impact: D-3-3 narrative cites `https://code.claude.com/docs/en/hooks`; T3-E-5 added to Research Insights; T3-I-T3.c cites the official doc; Scope Contract Decisions Locked updated to cite the official doc.

**F-4 — Finding 4 (COD-PROJ-001 — branch naming convention at row 5.5).** Picked option (c) — session-id-based branch name `session/{date}-{ssid-short}` not requiring task slug. Source of authority: iter1 COD-PROJ-001 (`evaluation/iter1/codex/project.md`); brief option (c). Rationale: preserves row 5.5 Scope-Contract lock; avoids rename complexity of (b); branch is short-lived so semantic name cost is bounded. Implementation impact: D-1 updated; T1-I-T1.a updated; G-1 scenario + E-2 scenario both updated.

**F-5 — Finding 5 (COD-STRUCT-001 — hook session-dir resolver).** Added D-3-3-resolver: project-name precedence `(ii)` read `$cwd/.gobbi/project.json` then `(i)` directory scan fallback; date prefix `(b)` scan `$cwd/.gobbi/projects/<name>/sessions/` for dir ending `-<session_id>`. Source of authority: iter1 COD-STRUCT-001 (`evaluation/iter1/codex/structure.md`); brief option (ii) + (b). Rationale: `project.json` is canonical when present; sessions-scan is clock-correctness-independent (option (a) `date +%Y-%m-%d` race; option (c) reads the file we're trying to find — circular). Implementation impact: D-3-3-resolver added; E-4 scenario refined; T3-I-T3.a updated.
