...
when-to-load: Manager loads before spawning a Codex evaluator (`Agent(codex:codex-rescue, ...)`) or directing the user toward `/codex:adversarial-review`. Subagents (leader/executor/evaluator/assistant) load before any inline `codex exec` Bash invocation. Delegation prompts that involve Codex MUST include this skill in the Load Directives block per `delegation/SKILL.md`.
---
...
<!-- Execution: fill. Cover these three use cases with worked examples:
  ...
  (1) Dual-system evaluator spawn.
      ...
      - Post-eval sanity check: `find /playinganalytics/git/gobbi/.gobbi/projects/gobbi/sessions/{session-id} -newer <marker> -type f` to confirm writes landed at the main-tree absolute path (per project mistake).
  ...
  Cost + sandbox budget awareness sub-bullet (folded in here, NOT a separate section):
...
<!-- Execution: fill. At least 8 anti-patterns with corrected approach per item:
  - Subagent attempts `Agent(subagent_type="codex:codex-rescue", ...)` — fails (no Agent tool). USE `codex exec` via Bash instead.
