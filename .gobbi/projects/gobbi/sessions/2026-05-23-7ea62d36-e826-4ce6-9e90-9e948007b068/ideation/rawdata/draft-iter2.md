...
Surgical revisions against aggregated iter1 EVALUATION findings (Claude PASS + Codex REVISE → aggregated REVISE). iter1 file preserved at `draft-iter1.md`. Each entry below cites the originating finding ID(s).

| Change | Finding ID(s) | Where |
|---|---|---|
| Witness precision: distinguished memorization-gap loops (T1=8, T2=13, T5=9 eval files — full evals + empty staging) from eval-also-skipped loops (T3=3, T4=2, T6=2, T7=2 eval files — partial evals + empty staging). Re-verified counts empirically with `find ... -type f \| wc -l`. | COD-PROJ-001 / COD-CONS-001 / COD-OVERALL-001 (High); F-CLAUDE-C-01 (Medium) | Framed Problem § Root cause #2; Research Insights I6 + I12; Implementation Checklist row 8 column "Anchored insight"; Decisions Log row 13 |
| Triple-symlink discipline: `.agents/skills/codex` directory symlink added alongside `.claude/skills/codex/SKILL.md` file symlink — codex must be loadable by codex itself. Verified `.agents/skills/` is a sibling of `.claude/skills/` containing 17 directory symlinks into `.gobbi/projects/gobbi/skills/*`. | COD-PROJ-002 (High) | Design A § "Symlink direction"; Implementation Checklist row 2; Decisions Log row 15 |
| Finding-Type vocabulary fixed: Step 2.5 classification re-spec'd against the **actual 5 Types** from `evaluation/SKILL.md:344-385`: `improvement`, `bug`, `scenario_gap`, `checklist_gap`, `design_flaw`. The terms `correction` and `decision-record` are NOT in the 5-Type set and have been removed. | COD-STRUCT-001 (High) | Design D § Step 2.5 specification "Classify each gap" subsection; Implementation Checklist row 8; Decisions Log row 7 (updated) |
| Mode default reconciled: Question 1 default changed from "Chat" to "auto" — matches existing `orchestration/templates/settings.default.json:3` `"mode": "auto"` verified via `jq '.mode'`. | COD-USAGE-001 (High) | Design G § Question 1; Decisions Log row 10 (updated) |
| Auto-backfill collision policy added: Step 2.5 auto-backfill mechanism now references `evaluation/SKILL.md § Slug + collision policy` (lines 385-393) and requires the assistant to (a) compute slug, (b) pre-write check for existing same-finding-id file (overwrite) vs different finding-id (numeric suffix disambiguation), (c) write only after the check. | COD-RISK-001 (High) | Design D § Step 2.5 specification "Auto-fill mechanical gaps" subsection; Implementation Checklist row 8 |
| Cost/budget subsection added: Codex skill now includes a "Cost + sandbox budget awareness" subsection covering token cost awareness, codex-vs-claude selection, default-to-sandbox recommendation. | COD-PERF-001 (Medium) | Design A § Section outline (new section 7); Implementation Checklist row 1 |
| Plugin path corrected: `.claude/plugins/...` → `~/.claude/plugins/...` at draft line 96 (home-rooted, the real path). Lines 144/149/154 already correct. | COD-AESTH-001 (Medium) | Framed Problem § Root cause #1 |
| Correction-memory citation corrected: `.claude/CLAUDE.md:33` → `.claude/CLAUDE.md:50` per `sed -n '50p'` verification. | COD-AESTH-002 (Low) | Framed Problem § Impact |
| Post-eval sanity check added (3rd corrective from mistake file): Codex skill Use Cases now includes a `find <session-dir> -newer ...` verification command to run after any codex evaluator completes, to confirm files landed at the correct absolute path. | F-CLAUDE-O-01 / F-CLAUDE-R-02 (Medium) | Design A § Section outline (Use cases); Implementation Checklist row 14 |
| Codex skill section count locked at **8**: (1) When to load, (2) Invocation patterns, (3) Why subagents must use `codex exec`, (4) Sandbox + CWD discipline, (5) Hang + timeout discipline, (6) Use cases, (7) Cost + sandbox budget awareness, (8) Anti-patterns. Plus Constraints block. All inconsistent counts in iter1 (5+, 6-7, 8) reconciled to 8. | F-CLAUDE-S-01 / F-CLAUDE-A-02 (Medium) | Design A § Section outline; Implementation Checklist row 1 |
| Settings.default.json defaults verified: `jq '.mode, .workflow.ideation.evaluate.mode, .git.pr'` confirms `"auto"`, `"always"`, `{"open": false, "draft": false}`. Design G assumptions now empirically grounded. | F-CLAUDE-R-03 (Medium) | Design G § Direction; Implementation Checklist row 13 |
| Plugin script line citations re-stamped: re-read `~/.claude/plugins/cache/openai-codex/codex/1.0.2/agents/codex-rescue.md` lines 9-13 and `~/.claude/plugins/cache/openai-codex/codex/1.0.2/skills/codex-cli-runtime/SKILL.md` lines 1-43. (No-op confirmation — citations already accurate after re-verification.) | F-CLAUDE-C-03 (Medium) | Framed Problem § Root cause #1; Research Insights I1 |
| Cross-link manifest added: New subsection in Decisions Log enumerating every cross-skill link this bundle creates (memorization↔mistake, delegation→memorization, wrap-up→evaluation, codex→mistake, codex→git). | F-CLAUDE-S-02 (Medium) | Decisions Log § Cross-link manifest (new subsection) |
| Success Criterion 8 citation: Verified the user has pre-approved the 7-item scope via the DISCUSSION lock; the `.claude/CLAUDE.md` rule cited is the spirit of the mistake/correction discipline rule at `.claude/CLAUDE.md:50` ("Every agent MUST load the `mistake` skill before starting work. When the user corrects any approach, immediately record it as a mistake in `.gobbi/projects/{name}/mistakes/`"). Wording softened to "user pre-approved via DISCUSSION lock; bundle respects the mistake-discipline rule per `.claude/CLAUDE.md:50`". | F-CLAUDE-U-02 (Medium) | Scope Contract § Success Criteria #8 |
...
**I14 (NEW iter2) — `.agents/skills/` is the codex-side mirror of `.claude/skills/`, populated with directory symlinks into `.gobbi/projects/gobbi/skills/`.**
Source: `ls -la /playinganalytics/git/gobbi/.agents/skills/` shows 17 entries, every one a directory symlink of the form `<skill-name> -> ../../.gobbi/projects/gobbi/skills/<skill-name>` (e.g., `delegation -> ../../.gobbi/projects/gobbi/skills/delegation`). The pattern is uniform.
Why it applies: For the new codex skill to be loadable **by codex itself** (the eat-your-own-dogfood principle), the source-of-truth directory at `.gobbi/projects/gobbi/skills/codex/` requires a `.agents/skills/codex -> ../../.gobbi/projects/gobbi/skills/codex` directory symlink alongside the `.claude/skills/codex/SKILL.md` file symlink. A codex skill that codex itself can't load is a contradiction the bundle must not ship. Item A is updated to require **both** symlinks (Decisions Log row 15 amended).
...
### Edge — Wrap-up detects a prior-loop staging gap (refined per iter2 finding-Type vocabulary)
...
2. Wrap-up WORK Step 2.5 (new) detects the gap and **classifies each finding** as `mechanical` or `judgment-required` per the classification rules (item D's specification using the actual 5 Types from `evaluation/SKILL.md:344-385`: `improvement`, `bug`, `scenario_gap`, `checklist_gap`, `design_flaw`).
3. Per item D's hybrid escalation shape:
   - **Mechanical findings** (finding's `Type` ∈ {`improvement`, `bug`, `scenario_gap`, `checklist_gap`} + single `Domain` value that routes deterministically per `evaluation/SKILL.md § Staging routing`) → Step 2.5 auto-writes the staging file from the finding's content **inline**, applying `evaluation/SKILL.md § Slug + collision policy` pre-write check (compute slug; check for existing file at destination; if same `finding-id`, overwrite; if different `finding-id`, disambiguate with `-2`, `-3` numeric suffix; record disambiguation in `rawdata/promotion-manifest.md`). No user gate.
   - **Judgment-required findings** (`Type` = `design_flaw` requiring routing arbitration, OR `disposition: open` requiring user arbitration, OR `Type`/`Domain` missing/unrecognized) → aggregated into a single NEEDS_CONTEXT surfacing with a `user-question:` block.
...
2. Per item D's Step 2.5 (new), Wrap-up later detects the staging gap and classifies it. For purely-mechanical gaps (deterministic routing using the actual 5 Types), Step 2.5 auto-backfills inline with the Slug + collision policy pre-write check. For judgment-required gaps, Step 2.5 surfaces a single NEEDS_CONTEXT.
...
### Adversarial (NEW iter2) — Codex skill ships missing `.agents/skills/codex` symlink → codex itself can't load the skill
...
| 8 | Edit `wrap-up/SKILL.md`: insert "Step 2.5 — Prior-loop memorization compliance check" between current Steps 2 and 3 (`wrap-up/SKILL.md:137-138`). Include: inputs (rawdata + staging across prior loops), scan procedure, gap-classification table (**zero-staging, naming-shape, missing-template, directory-absent** — the 4th category per F-CLAUDE-C-01) with each gap categorized as `mechanical` (Type ∈ {`improvement`, `bug`, `scenario_gap`, `checklist_gap`} + single Domain → deterministic routing per `evaluation/SKILL.md § Finding Metadata` lines 344-385) or `judgment-required` (Type = `design_flaw` routing arbitration, `disposition: open` user arbitration, Type/Domain missing/unrecognized, or multi-subdir spans → aggregate NEEDS_CONTEXT); auto-backfill mechanism with `evaluation/SKILL.md § Slug + collision policy` pre-write check (lines 385-393: compute slug; check existing same-finding-id → overwrite; different finding-id → numeric suffix); NEEDS_CONTEXT trigger with `user-question:` block schema. Update Exit checklist (`wrap-up/SKILL.md:166-174`). | I7 + I11 + iter1-user-redirects § Decision 1 | `grep "Step 2.5\|prior-loop memorization compliance" wrap-up/SKILL.md` returns the new section; `grep "mechanical\|judgment-required" wrap-up/SKILL.md` returns ≥ 1 each; `grep "Slug + collision\|finding-id" wrap-up/SKILL.md` confirms collision policy referenced; `grep "improvement\|bug\|scenario_gap\|checklist_gap\|design_flaw" wrap-up/SKILL.md` confirms 5-Type vocabulary |
...
**Symlink direction (concern 4 — resolved, EXPANDED per iter2 / COD-PROJ-002)**:
...
- **Symlink 2 (Codex surface)**: `.agents/skills/codex -> ../../.gobbi/projects/gobbi/skills/codex` (directory-level symlink — matches existing 17-entry pattern verified via `ls -la .agents/skills/`; every entry is a directory symlink of this exact form).
...
**Step 2.5 specification (restructured per iter1 user redirect § Decision 1 — hybrid escalation; finding-Type vocabulary corrected per COD-STRUCT-001; collision policy added per COD-RISK-001; directory-absent gap added per F-CLAUDE-C-01)**:
...
- **Procedure**:
  ...
  4. **Classify each gap** using the **actual 5-Type vocabulary** from `evaluation/SKILL.md:344-385`:
     - **`mechanical`** = the finding's `Type` ∈ {`improvement`, `bug`, `scenario_gap`, `checklist_gap`} AND has a single `Domain` value AND routes deterministically to one staging subdir per `evaluation/SKILL.md § Staging routing`. Example: an `improvement` finding tagged Domain=`testing` with `disposition: addressed` routes uniquely to `staging/improvements/{slug}.md`.
     - **`judgment-required`** = ANY of: the finding's `Type` is `design_flaw` (routing arbitration belongs to the user); the finding has `disposition: open` requiring user arbitration; the finding's `Type` or `Domain` is missing or unrecognized; the finding spans multiple staging subdirs.
     - **Note**: the terms `correction` and `decision-record` (used in iter1 draft) are NOT in the 5-Type vocabulary. Corrections / decisions / mistake-candidates surface as `improvement` or `design_flaw` findings + Domain tags + `mistake-candidate: true` frontmatter; they are not first-class Types.
...
Captured from DISCUSSION (see `sessions/.../ideation/rawdata/discussion-log.md`) plus this WORK phase plus the post-WORK iter1 user redirects (see `sessions/.../ideation/staging/decisions/iter1-user-redirects.md`) plus iter2 aggregated-eval revisions.
...
| 7 | Step 2.5 escalation shape | **Hybrid: auto-backfill mechanical gaps inline (deterministic Type+Domain routing per `evaluation/SKILL.md § Finding Metadata` lines 344-385 using the actual 5 Types: `improvement`, `bug`, `scenario_gap`, `checklist_gap`, `design_flaw`) + aggregate NEEDS_CONTEXT for judgment-required gaps (Type=`design_flaw` routing arbitration, `disposition: open` user arbitration, missing/unrecognized Type/Domain, multi-subdir spans). Auto-backfill respects `evaluation/SKILL.md § Slug + collision policy` lines 385-393 (stable finding-id, pre-write check, same-id overwrite, different-id suffix disambiguation).** Classification rules documented in `wrap-up/SKILL.md` for auditability. | iter1-user-redirects § Decision 1 (post-WORK user redirect); iter2 corrections per COD-STRUCT-001 + COD-RISK-001 |
...
| 15 | Symlink direction (concern 4 — resolved; EXPANDED iter2 per COD-PROJ-002) | **Source-of-truth at `.gobbi/projects/gobbi/skills/codex/SKILL.md`; TWO symlinks**: (a) `.claude/skills/codex/SKILL.md -> ../../../.gobbi/projects/gobbi/skills/codex/SKILL.md` (Claude file-level), (b) `.agents/skills/codex -> ../../.gobbi/projects/gobbi/skills/codex` (Codex directory-level, matching the 17-entry pattern verified via `ls -la .agents/skills/`). **Both are required**: a codex skill that codex itself cannot load is a contradiction. | iter1-user-redirects § "Concerns resolved without user input" + iter2 per COD-PROJ-002 + I14 |
| 16 (NEW iter2) | iter2 changelog discipline | Every iter2 revision is logged in the top-of-file Iter2 Changelog table with originating finding ID, change summary, and target sections. iter1 file preserved at `draft-iter1.md`. | iter2 leader brief constraint |
| 17 (NEW iter2) | Codex skill section count locked at 8 | Resolved iter1's inconsistent counts (5+/6-7/8) to a single locked value of 8 H2 sections: When to load / Invocation patterns / Why subagents must use `codex exec` / Sandbox + CWD / Hang + timeout / Use cases / Cost + sandbox budget awareness / Anti-patterns. Plus Constraints block. | F-CLAUDE-S-01 / F-CLAUDE-A-02 |
| 18 (NEW iter2) | Mistake-corrective coverage in codex skill | All THREE correctives from `codex-eval-session-write-path-nested-in-worktree.md` must appear in the codex skill: (1) absolute-path mandate in delegation prompt, (2) `--cd` set to main-tree absolute, (3) post-eval `find` sanity check. iter1 captured (1)+(2) only; iter2 adds (3) to Design A § Section outline (Use cases) and Implementation Checklist row 14. | F-CLAUDE-O-01 / F-CLAUDE-R-02 |

### Cross-link manifest (NEW iter2 per F-CLAUDE-S-02)
...
| 4 | `wrap-up/SKILL.md § Step 2.5` → `evaluation/SKILL.md § Finding Metadata (lines 344-385)` | Type vocabulary source for mechanical classification. | D |
...
| 6 | `wrap-up/SKILL.md § Step 2.5` → `evaluation/SKILL.md § Staging routing` | Deterministic-routing source for mechanical destinations. | D |
...
**Status update (post-iter2 revision)**: All Codex-High findings (COD-PROJ-001/002, COD-CONS-001, COD-STRUCT-001, COD-USAGE-001, COD-RISK-001, COD-OVERALL-001) are addressed inline via this iter2 surgical revision. All Claude-Medium findings flagged in the leader brief (F-CLAUDE-C-01, F-CLAUDE-O-01/R-02, F-CLAUDE-S-01/A-02, F-CLAUDE-R-03, F-CLAUDE-C-03, F-CLAUDE-S-02, F-CLAUDE-U-02) are addressed. Only the original Item E text concern remains pending — it is a Planning-phase clarification (non-blocking for ideation evaluation).
...
End of draft-iter2 (surgical revision against aggregated iter1 EVALUATION findings).
