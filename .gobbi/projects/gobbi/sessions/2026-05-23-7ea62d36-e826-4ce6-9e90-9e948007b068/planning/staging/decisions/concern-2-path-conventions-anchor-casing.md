...
`memorization/SKILL.md:224` heading is `**Path conventions**` (lowercase 'c', bold paragraph). Cross-Link 7 requires a stable anchor target. Options: (a) cross-link to the bold paragraph as-is (no stable anchor); (b) promote to `### Path conventions` H3.
...
These are NOT promoted in this session (Concern 2 only targets the memorization site that Cross-Link 7 needs). A follow-up backlog entry will be staged at `backlogs/project/normalize-path-conventions-h3.md` by Task 05 to track the cross-skill normalization for a future session.

## Evidence

- `memorization/SKILL.md:224` (verified via `sed -n '220,235p'`) — current `**Path conventions**` bold paragraph.
...
Adopted in Planning draft-iter1.md Task 05. Backlog spillover entry staged in same task. No User Challenge.
