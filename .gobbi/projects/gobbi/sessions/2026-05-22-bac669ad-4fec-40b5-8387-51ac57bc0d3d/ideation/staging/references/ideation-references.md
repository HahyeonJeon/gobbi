...
The relevant hook stdin schema (paraphrased from the official docs, as retrieved by the assistant subagent):
...
Version entry (paraphrased per Codex evaluator's citation): Claude Code v2.1.132 introduced `CLAUDE_CODE_SESSION_ID` as a runtime-auto-set environment variable containing the current session UUID.
