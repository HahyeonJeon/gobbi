...
# Captured: 2026-05-30
# To re-run: copy this file back to plan.md

# Planning — gobbi v0.5 Claude Code Plugin (install-runtime)

**Loop:** planning · iter-1 · **Session:** 0fd65721-c39f-4305-b296-9961aee8e1c1
...
This plan decomposes two coupled, ratified deliverables (A: the bounded `gobbi` plugin package; B: distribution + tooling + docs) into 8 ordered, single-category Execution tasks. All design decisions (DD-1..DD-9 + the 5 resolved design-details + the 18-skill / 5-agent / 2-script-3-registration inventory) are RATIFIED upstream and are NOT re-opened here.
...
- **A.** Bounded `gobbi` plugin package at `plugins/gobbi/` = `.claude-plugin/plugin.json` + `skills/` (18 real-copy dirs) + `agents/` (5 real-copy `.md`) + `hooks/` (2 real-copy scripts + `hooks.json`), nothing else.
- **B.** Repo-root `.claude-plugin/marketplace.json` (Claude schema) + `scripts/sync-plugin-package.sh` (materialization + drift gate) + fire-exactly-once hook validation + post-install invocability check + conditional `.claude/settings.json` permission entries + `claude-plugin` skill (+ `.claude/skills/` mirror symlink) + feature-memory doc updates.
...
- 18 skills packaged (incl. `gobbi-hook-authoring`); 5 `.md` agents only (exclude `.toml`); 2 scripts / 3 registrations.
...
|---|---|---|
| `scripts/sync-plugin-package.sh` | Create | Re-materialize `plugins/gobbi/{skills,agents,hooks}/` from canonical sources as REAL copies; `--check` diff/checksum gate that exits non-zero on divergence. |
| `plugins/gobbi/skills/**` | Create | 18 real-copy skill dirs (every file under each), output of the sync script. |
...
### Concern 5 — Fire-exactly-once hook validation (T5)
| File | Op | Responsibility |
|---|---|---|
| `scripts/validate-plugin-hooks-fire-once.sh` | Create | Installed-case fire-once validator: per-fire marker keyed on `hook_event_name`; deterministic per-event triggers; assert exactly one marker per event. |

### Concern 6 — Post-install invocability check + conditional permissions (T6)
| File | Op | Responsibility |
|---|---|---|
| `scripts/check-plugin-invocability.sh` | Create | Post-install check that invokes `gobbi:codex` + `gobbi:gobbi-hook-authoring` + one agent; reports auto-grant TRUE/FALSE. |
| `.claude/settings.json` | Modify (CONDITIONAL) | Add `Skill(codex)` + `Skill(gobbi-hook-authoring)` to `permissions.allow` ONLY IF the check proves auto-grant FALSE. |

### Concern 7 — `claude-plugin` skill + mirror (T7)
| File | Op | Responsibility |
|---|---|---|
| `.gobbi/projects/gobbi/skills/claude-plugin/SKILL.md` | Create | Canonical `claude-plugin` skill: general Claude-Code-plugin authoring/update guide + layered gobbi-specific section. |
...
what: Author scripts/sync-plugin-package.sh (materialize + --check diff gate) and run it to produce the plugins/gobbi/{skills,agents,hooks}/ real-copy component trees (18 skill dirs, 5 .md agents, 2 hook scripts +x).
traces-to:
...
verifies: >
  Run scripts/sync-plugin-package.sh then: (a) plugins/gobbi/skills/ has exactly 18 dirs matching the canonical set;
  (b) plugins/gobbi/agents/ has exactly the 5 .md and zero .toml; (c) the 2 hook scripts are real files (test ! -L), +x preserved,
  bodies byte-identical to canonical (diff -r returns empty); (d) every file under plugins/gobbi/{skills,agents,hooks} is a real file, find -type l returns empty;
  (e) scripts/sync-plugin-package.sh --check exits 0 immediately after a sync and exits non-zero after a deliberate single-byte tamper.
```
...
what: Author scripts/validate-plugin-hooks-fire-once.sh and run it in an installed-only environment to assert each of the 3 registrations fires exactly once, keyed on hook_event_name, with a deterministic per-event trigger (incl. a non-zero-exit Task for PostToolUseFailure).
traces-to:
...
outputs: [fire-once-validator, fire-once-result]
verifies: >
  Install the plugin from the worktree-faithful path (DD-7: commit/push the worktree branch, add the marketplace from a git-ref
  source pointing at that branch, install) into a clean installed-only environment with NO active in-repo .claude/settings.json dev
  registrations; run scripts/validate-plugin-hooks-fire-once.sh; assert the marker log contains EXACTLY one line per hook_event_name
  (SessionStart, PostToolUse, PostToolUseFailure), where PostToolUseFailure is triggered by a Task engineered to exit non-zero;
  also assert the worktree-only sentinel is present in ~/.claude/plugins/cache/<id>/ (DD-7 falsifier).
```

```yaml
id: 06-invocability-check-and-conditional-permissions
what: Author scripts/check-plugin-invocability.sh, run it post-install to test the auto-grant premise by invoking gobbi:codex + gobbi:gobbi-hook-authoring + one agent, and — only if auto-grant proves FALSE — add Skill(codex)+Skill(gobbi-hook-authoring) to .claude/settings.json permissions.allow.
traces-to:
...
verifies: >
  With the plugin installed (from T5's worktree-faithful install), run scripts/check-plugin-invocability.sh; it attempts to
  invoke gobbi:codex, gobbi:gobbi-hook-authoring (the 2 skills OMITTED from the live allow-list), and one of the 5 agents (e.g. leader),
  and records auto-grant TRUE (all load) or FALSE (any refused). IF FALSE: python3 -m json.tool .claude/settings.json parses AND
  permissions.allow now contains both Skill(codex) and Skill(gobbi-hook-authoring), and no other allow entry changed (diff the allow array
  against the pre-edit snapshot — exactly +2). IF TRUE: .claude/settings.json is UNCHANGED (the conditional Modify does not fire).
```

```yaml
id: 07-claude-plugin-skill-and-mirror
what: Author the canonical .gobbi/projects/gobbi/skills/claude-plugin/SKILL.md (general Claude-Code-plugin authoring/update guide + layered gobbi section) and create the .claude/skills/claude-plugin/SKILL.md mirror symlink.
traces-to:
...
outputs: [claude-plugin-skill, claude-plugin-mirror-symlink]
verifies: >
  ...
  re-sync trigger/gate, the dev-vs-installed hook split + double-fire caveat, the agents-REPLACES vs skills-ADDS-to asymmetry, the
  symlink-skip footgun, version cadence, and the validate/install/update flow. readlink .claude/skills/claude-plugin/SKILL.md resolves
  to the canonical target and `test -e` on the resolved path succeeds (symlink not dangling).
```
...
verifies: >
  features/install-runtime/README.md now references: (a) the gobbi plugin package at plugins/gobbi/ + repo-root marketplace.json;
  (b) the DD-8 Option-C dev-vs-installed hook split (settings.json dev + plugin hooks.json installed); (c) the materialized-copy
  re-sync trigger + scripts/sync-plugin-package.sh gate; (d) a pointer to the new claude-plugin skill. last_updated bumped to 2026-05-30.
...
| 05 fire-once validation | 03, 04 | 06, 07 | `scripts/validate-plugin-hooks-fire-once.sh` |
| 06 invocability + perms | 05 | 07 | `scripts/check-plugin-invocability.sh`, `.claude/settings.json` (conditional) |
| 07 claude-plugin skill + mirror | 01,02,03,04,05,06 | 08 | `.gobbi/.../skills/claude-plugin/SKILL.md`, `.claude/skills/claude-plugin/SKILL.md` |
| 08 feature README | 07 | — | `features/install-runtime/README.md` |

No two tasks create or modify the same file. The only shared parent dirs (`plugins/gobbi/`, `scripts/`) hold disjoint files per task. Acyclic; bottom-up (package base → manifest/hooks → catalog → runtime checks → docs).
...
- **07 near-last** — the `claude-plugin` skill documents the *finished* package shape including the empirically-resolved hook double-fire behavior (T5) and auto-grant finding (T6); writing it earlier would document unverified premises (Principle 7).
- **08 last** — Principle 8: docs ship reflecting the final, verified implementation.
...
| L2 (runtime verification) | 05 → 06 | sequential, after L1 |
| L3 (documentation) | 07 → 08 | sequential, after L1+L2 |

Lanes are documentation only — the Execution Loop runs all 8 tasks strictly sequentially (01→08). 02 and 03 both depend only on 01 and touch disjoint files, so they are parallel-safe in principle, but Execution sequences them. **Conflict flags: none** — no two tasks touch the same file.
...
| 05 | executor | sonnet | `principles`, `execution`, `gobbi-hook-authoring`, `git` | `executor-cwd-reset-commits-task-to-wrong-branch`, `reproducing-a-bugged-command-is-not-validation`, `subagent-relative-write-paths-stray-cd-doesnt-persist` |
| 06 | executor | sonnet | `principles`, `execution`, `git` | `executor-cwd-reset-commits-task-to-wrong-branch`, `subagent-relative-write-paths-stray-cd-doesnt-persist` |
...
- T1/T7 carry the symlink/mirror + worktree-copy mistakes because they are the two tasks that touch the materialize-vs-symlink distinction (T1 makes real copies; T7 makes a mirror symlink) — the exact surface `skills-mirror-symlinks-not-copies` and `executor-mirror-path-vs-worktree-physical-copy` warn about conflating.
- T3/T5 carry `gobbi-hook-authoring` because they author/validate hook registration shape.
- T4/T5/T6 carry `git` + `executor-cwd-reset-commits-task-to-wrong-branch` because DD-7's install-test requires commit/push of the worktree branch — the highest-risk cwd/branch surface.
...
| DECIDE hook double-registration + fire-exactly-once validation | RATIFIED (Option C) upstream; validation = 05 |
| DECIDE permissions disposition + invocability check | RATIFIED upstream; check + conditional entries = 06 |
| Author marketplace.json (name source value) | 04 (`source: "./plugins/gobbi"`) |
| DECIDE worktree-faithful install/test path + sentinel | RATIFIED (Option a) upstream; exercised in 05 |
| Post-install cache-contents gate (allow-set skills/agents/hooks only) | folded into 01's `--check` (real-files-only assertion) + exercised at 05 install (allow-set verified post-install) |
| Author claude-plugin skill + mirror symlink | 07 |
| Verify: validate --strict / install / cache gate / fire-once / readlink + section-presence | distributed across task `verifies` (04 validate, 05 fire-once+sentinel, 06 invocability, 07 readlink+sections) |
...
**Cache-contents gate placement note:** the readiness doc's allow-set gate (`{.claude-plugin/, skills/, agents/, hooks/}` post-install) is asserted at the T5 install step's environment (the installed cache must contain only those subtrees) and pre-install by T1's real-files-only `--check`. It is not a standalone task because it has no independent deliverable file beyond what T1's gate and T5's install assertion already produce. Flagged for the evaluator.
...
**Result: zero findings.** No drift, full coverage, no placeholders.

---

## Principle-13 SPEC + CRUD for the documentation tasks (T7, T8)

### T7 — `claude-plugin` skill + mirror symlink
**SPEC:** Create a new project skill that (a) teaches general Claude-Code-plugin authoring/update and (b) carries a layered gobbi-specific section documenting this package's bounded layout, materialization re-sync trigger/gate, DD-8 dev-vs-installed hook split + double-fire caveat, the `agents`-REPLACES vs `skills`-ADDS-to asymmetry, the escaping-symlink-skip footgun, version cadence, and validate/install/update flow. Memory type: **skill** (canonical under `.gobbi/.../skills/`, workspace-visible via `.claude/skills/` mirror symlink). Must NOT bleed into: a feature `design` doc (this is reusable cross-feature authoring guidance, not install-runtime design) or a `mistake` (it is a how-to, not a correction record).
**CRUD:**
...
- **Update** none.
- **Delete** none.
- **Blast radius:** the canonical skill is NOT mirrored anywhere except the one `.claude/skills/` symlink (the mirror is a symlink layer — ONE canonical edit, see `skills-mirror-symlinks-not-copies`). It is NOT auto-listed in `CLAUDE.md` navigation; the readiness doc explicitly scopes adding it to a follow-up (the FLAG-2 `claude` dangling-reference observation is separate). **Do NOT add this skill to `.claude/settings.json` permissions.allow** — that is T6's conditional surface and out of scope for T7. **Do NOT add to the plugin package** — the `claude-plugin` skill is authoring-guidance, and packaging-vs-not was not ratified for it; the 18-skill inventory is frozen (T1) and does not include `claude-plugin`. (If packaging it is desired, that is a follow-up, not in this plan's scope.)

### T8 — feature README update
**SPEC:** Update the `install-runtime` feature README so the feature overview reflects the shipped plugin. Memory type: **feature README** (overview/index of feature memory). Must contain: a short subsection or Recent-activity row noting the gobbi plugin package, the dev-vs-installed hook split, the materialization re-sync surface, and a pointer to the new `claude-plugin` skill. Must NOT bleed into: re-deriving design decisions (those live in `decisions/`), or duplicating the skill body.
**CRUD:**
- **Update** `.gobbi/projects/gobbi/features/install-runtime/README.md` — add the plugin to the Subsystems/Overview prose (1 subsystem line + 1-2 overview sentences), add a Recent-activity row (`2026-05-30 | 0fd65721 | gobbi v0.5 Claude Code plugin package shipped`), bump `last_updated: 2026-05-30`.
...
- **Implementing the `scripts/sync-plugin-package.sh` mechanism choice** beyond what T1 produces — T1 ships a working script + gate; build-vs-CI-vs-tracked is the executor's mechanism call within T1.
...
- **Packaging the new `claude-plugin` skill into `plugins/gobbi/skills/`** — the 18-skill inventory is frozen and excludes it; follow-up if desired.
...
|---|---|---|---|
| D-1 | Task count + ordering | 8 sequential tasks, bottom-up (package base → manifest/hooks → catalog → runtime checks → docs) | leader decomposition (this plan) |
...
| D-3 | Cache-contents allow-set gate has no standalone task | Folded into T1 `--check` (real-files-only) + asserted at T5 install env; no independent deliverable file | readiness doc + Ideation checklist |
...
| D-5 | T6 permissions edit is CONDITIONAL | DD-9 ratified: only add Skill() entries IF the invocability check proves auto-grant FALSE | `permissions-disposition-keep-project-local-verify-empirically.md` |
...
| D-7 | DD-7 install-test exercised inside T5 (not a standalone task) | The git-ref-source + sentinel install is the precondition for fire-once + invocability; no separate deliverable | `worktree-test-default-git-ref-source-with-sentinel.md` |
...
**USER CHALLENGE escalations:** none — the leader's decomposition does not substantively disagree with any ratified Ideation/Preparation decision.

**NEEDS_CONTEXT:** none — every design choice required to plan was already ratified upstream.
