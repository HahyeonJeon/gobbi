...
related:
  - ideation/staging/decisions/2026-05-30-permissions-auto-grant-assumption.md
---
...
- **Tag the auto-grant premise as assumption_risk; resolve it empirically** via the post-install invocability check. The check MUST invoke the components that actually test the auto-grant premise — i.e. the 2 skills NOT in the live allow-list (`gobbi:codex` and `gobbi:gobbi-hook-authoring`) plus one of the 5 agents (e.g. `leader`). Invoking an already-allowed skill would not falsify the premise. If they load without an explicit allow entry → auto-grant TRUE; if refused → auto-grant FALSE.
...
- plugin-manifest reference (no auto-grant statement in docs).
