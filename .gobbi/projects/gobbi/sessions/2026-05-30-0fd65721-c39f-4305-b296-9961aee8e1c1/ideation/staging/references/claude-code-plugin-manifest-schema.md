...
## Why it applies
This is the exact schema the `.claude-plugin/plugin.json` deliverable must conform to. It settles S1 (the
`agents` key is a file-path array, so the plugin enumerates exactly the 5 role `.md` files and excludes the
`.toml` Codex wrappers) and the ADDS-to vs REPLACES asymmetry the `claude-plugin` skill must teach.
