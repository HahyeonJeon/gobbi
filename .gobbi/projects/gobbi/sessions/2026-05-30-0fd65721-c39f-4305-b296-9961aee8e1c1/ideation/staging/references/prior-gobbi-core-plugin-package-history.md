...
- **62b95a0 (PR #6)** introduced `.claude-plugin/marketplace.json` (`name: gobbi`, plugins[]→`gobbi-core`,
  ...
  the bounded-package model the user re-chose in iter-2. The initial manifest used directory pointers:
...
- **ba8aa42** (`fix(marketplace): remove invalid hooks and agents fields from plugin.json`) PROVED the old
  ...
  paths — see the manifest-schema reference — so the iter-2 fix is the 5-file array, not omission.)
...
## Why it applies
Corrects the iter-1 false claim "No prior `.claude-plugin/` attempt on record." Planning must start from the
...
matcher-too-narrow hook footgun (#256). The user's iter-2 direction (fresh build, mine history for proven
solutions) maps 1:1 onto these lessons.
