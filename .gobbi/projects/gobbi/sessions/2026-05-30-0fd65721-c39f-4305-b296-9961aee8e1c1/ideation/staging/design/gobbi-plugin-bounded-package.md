...
related:
  ...
  - ideation/staging/decisions/2026-05-30-bounded-package-root-path-unnamed.md
  - ideation/staging/decisions/2026-05-30-drift-sync-resync-trigger-unnamed.md
---
...
The package is a bounded subtree (parent path TBD — see `staging/decisions/2026-05-30-bounded-package-root-path-unnamed.md`):

```
<package-root>/
  .claude-plugin/
    plugin.json          # manifest (name: gobbi, metadata, skills, agents, hooks)
    marketplace.json     # Claude-schema marketplace (name/owner/plugins[])
...
**Trade-off named (from #251):** "Editing on main now requires editing in two places." This creates a canonical-tree↔package-copy drift surface. The `claude-plugin` skill MUST document: (a) the keep-in-sync obligation; (b) a named re-sync trigger condition; (c) a recommended mechanical enforcement path (build/sync step or diff gate).

### Install path

In-repo Claude-schema `marketplace.json` + `/plugin marketplace add` + `/plugin install`. The worktree-faithful test path (DD-7) must use a method that proves the installed cache came from the worktree branch, not the main checkout (relative marketplace sources resolve to main from a worktree).

### Hook double-registration (DD-8 — Planning blocker)

Options: (A) plugin REPLACES project-local; (B) coexist (not recommended — double env-export + competing session.json upserts); (C) conditional/dev-vs-installed split. Recommended: Option A. Requires fire-exactly-once validation across all 3 registrations post-install.

## Scenarios

Key scenarios (full enumeration in `staging/scenarios/`):

- **Golden — install fresh (worktree-faithful):** skills appear namespaced `gobbi:<skill>`, 5 agents invocable, 3 hook registrations each fire once.
- **Edge — cache-contents boundary (R1):** post-install cache contains ONLY skills/agents/hooks (no sessions/, no project memory).
- **Edge — agents field shape (S1):** 5-`.md` array; `.toml` wrappers absent; validate passes; agents invocable.
- **Edge — drift/sync (DD-2a):** canonical skill edit NOT re-synced → cache is stale. Skill documents prevention.
- **Failure — escaping symlinks (proven #251):** symlink-based package → empty plugin on install.
- **Failure — directory agents/hooks (proven ba8aa42):** directory-valued fields fail schema validation.
- **Failure — too-narrow matcher (proven #256) / double-fire (R2):** matcher that misses an event source, or both plugin + project-local registering → duplicate behavior.
- **Failure — worktree tests wrong checkout (U1):** naive `./` install from worktree loads main state.

## Validation

| Criterion | Verification method |
|---|---|
| Manifest validity | `claude plugin validate --strict ./<package>` |
| Cache-contents gate | Enumerate `~/.claude/plugins/cache/<id>/`; assert allow-set = skills/agents/hooks only |
...
**Sacrifices:** single source of truth (canonical tree and package are two copies, not one). Mitigated by naming a re-sync trigger and recommending a mechanical sync gate. The #258 drift-detector backlog is the long-term solution.

## Open Issues

1. **Package root path and marketplace `source` value not named** (STRUCT-1 — Planning must resolve; see `staging/decisions/2026-05-30-bounded-package-root-path-unnamed.md`).
2. **Drift/sync re-sync trigger not specified** (F-S1 / CONS-1 — Planning must specify; see `staging/decisions/2026-05-30-drift-sync-resync-trigger-unnamed.md`).
3. **DD-7 worktree test path lacks recommended default** (F-U1 — Planning should ratify option (a); see `staging/scenarios/worktree-faithful-install-path-default.md`).
4. **DD-9 permissions auto-grant unverified** (F-P1 — Planning should tag as assumption_risk and use invocability check as falsifier; see `staging/decisions/2026-05-30-permissions-auto-grant-assumption.md`).
