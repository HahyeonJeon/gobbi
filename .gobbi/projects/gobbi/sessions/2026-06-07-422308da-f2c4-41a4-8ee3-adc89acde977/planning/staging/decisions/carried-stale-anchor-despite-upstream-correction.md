...
Red flag: a plan's T-n verification block cites a file line number that the leader did not re-verify against the readiness report for that task. Escalating signal: both evaluators independently flag the same line anchor as wrong in the first review iteration — this means the mistake is mechanical, not subtle. Related patterns: [[planning-leader-asserted-file-type-without-verifying]] (asserting a file property without a fresh read), [[leader-iter2-verification-claim-without-evidence]] (making a no-survivor claim without a fresh grep).

## Related

- `.gobbi/projects/gobbi/mistakes/` — load these at Planning start.
- [[planning-leader-asserted-file-type-without-verifying]] — same failure mode: a property asserted from prior context rather than re-verified.
- [[leader-iter2-verification-claim-without-evidence]] — self-review no-survivor claim that was literally false.
