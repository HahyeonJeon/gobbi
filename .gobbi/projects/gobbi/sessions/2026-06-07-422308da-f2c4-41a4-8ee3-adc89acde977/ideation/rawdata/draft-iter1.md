...
- **D3:** Problem-3 root cause CONFIRMED — it is ONE root cause (mode-agnostic routine-triage escalation) with FOUR in-scope instances: CLAUDE.md "discuss findings" (3a) + evaluation.md § Iteration Caps (3b) + § Stuck detection (3c, Codex iter2) + § Regression marking (3d). All four must be mode-split. §7 hardening alone is insufficient.
...
## Evaluation summary
(to be filled by assistant at MEMORIZATION on PASS)
