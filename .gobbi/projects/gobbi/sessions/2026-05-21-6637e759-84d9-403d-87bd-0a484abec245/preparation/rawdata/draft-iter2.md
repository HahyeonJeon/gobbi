# Preparation Draft — iter2

> **iter2 surgical delta** over iter1: adds the `## Pre-routed gaps for Planning` section between "Out of scope gaps" and "Decisions log" (per the Codex iter1 evaluator's 2 findings), and updates the Readiness summary + Decisions log to cite the iter2 round outcome. **All other sections are preserved verbatim from iter1.** No re-litigation of the 19 user-locked Ideation decisions; no scope rewrites.
...
Gap count: **0 Preparation-blocking; 2 pre-routed for Planning (1 high + 1 medium)**. Generated this loop: **0** skills, **0** memory promotions. Deferred to Planning (already flagged in Ideation handoff): **1** finding (F-CX-O4-01 `--delete-branch` wording) — this is not a Preparation gap; Ideation already routed it to Planning.

No `re-ideate` triggers. Preparation can advance directly to EVALUATION → Planning, with the two new pre-routed constraints below binding on Planning's decomposition.
...
**Recommendation**: Planning's Stage B task description (or equivalent inventory line) explicitly enumerates both deletions in the "already-deleted-in-tree" sub-bullet. One-line correction.
...
- Consolidated gap table: **empty** for Preparation-blocking gaps. No row required user AskUserQuestion routing for `generate-now` / `re-ideate` / `skip` in iter1.
...
### iter2 round outcome (this iteration)

- **Trigger**: iter1 dual-system EVALUATION reconciliation surfaced 2 Codex findings (F-CX-PREP-O-01 High/75 + F-CX-PREP-O-02 Medium/75) that the iter1 Claude evaluator marked PASS and missed. Cross-system divergence rule (pessimistic union per evaluator skill) honors the Codex findings.
- **Manager AskUserQuestion**: presented the 2 findings to the user with options (full REVISE re-discussion vs. surgical iter2 additive routing). User authorized **surgical iter2** with a new `## Pre-routed gaps for Planning` section that documents both findings as binding constraints on Planning, without re-litigating the 19 user-locked Ideation decisions or rewriting other sections.
- **Iter2 scope (in)**: add `## Pre-routed gaps for Planning` between "Out of scope gaps" and "Decisions log"; update Readiness summary to reflect "1 high-severity Planning-relevant constraint surfaced + 1 medium doc-sync"; add this iter2-round-outcome subsection citing the Codex findings + user-locked surgical fix.
- **Iter2 scope (out)**: re-litigating any of the 19 locks; rewriting Scope reference / Design + memory readiness / Execution skills readiness / Generated this loop / Out of scope gaps sections; expanding scope beyond the 2 Codex findings.
- **Audit trail**: iter1 draft preserved verbatim at `rawdata/draft-iter1.md`; iter2 is a new file at `rawdata/draft-iter2.md` per the user-authorized routing.

**Verification commands run during this loop** (and their outputs that supported the iter1 empty-gap conclusion — preserved verbatim from iter1, still valid in iter2):
...
All preconditions met. Preparation Loop iter2 verdict candidate: **PASS** (with 2 pre-routed Planning constraints documented in the new section above) — pending EVALUATION.

### Notes for downstream EVALUATION

- **Gap coverage**: every Sub-step B/C surface was scanned in iter1; the 2 Codex iter1 findings now sit in the `## Pre-routed gaps for Planning` section as binding Planning constraints (not Preparation `generate-now` gaps). Iter2 evaluator should verify the routing is sound — i.e., the findings are addressable at the Planning boundary and do not require a Preparation `re-ideate`.
- **Generation quality**: not applicable — zero artifacts generated this loop (iter2 surgical-add scope confirms this).
- **`re-ideate` triggering**: not applicable — F-CX-PREP-O-01 and F-CX-PREP-O-02 are both Planning-routable (decomposition choice + checklist doc-sync respectively), not unworkable Ideation gaps.
- **Scope discipline**: iter2 stays additive — Scope reference / Design + memory readiness / Execution skills readiness / Generated this loop / Out of scope gaps sections are unchanged from iter1; only Readiness summary + Decisions log received minor edits per the user-authorized surgical scope.
- **NEEDS_CONTEXT**: none required from the user this loop beyond the iter2-routing AskUserQuestion the manager already ran.
