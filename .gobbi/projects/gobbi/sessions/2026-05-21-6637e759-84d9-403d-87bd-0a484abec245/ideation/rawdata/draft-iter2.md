# Ideation rawdata — iter 2 (FINAL — 15 locks + 4 High remediations + 4 Med/Low surgical fixes)
...
This iter2 draft inherits all 15 locked decisions from iter1 verbatim and applies the iter1 Claude evaluator's four High-severity remediations (per the manager's iter2 AskUserQuestion round) plus four Medium/Low surgical fixes the evaluator surfaced. iter1 stays untouched as the audit trail at `draft-iter1.md`.

**iter2 deltas at a glance** (changes from iter1 — locked decisions unchanged):

- **H-1 (F-P-01)** — Add Stage B step to edit `.claude/CLAUDE.md` removing lines 61-62 (the two `v050-{overview,cli}.md` table rows). The two design files themselves get deleted via the existing `rm -rf design/*` in Stage C.
- **H-2 (F-R-02)** — Decisions Log records the user-accepted trade-off: the 3 promoted mistake files in `mistakes/` (`executor-rationalized-failing-verification-gate.md`, `session-dir-naming-convention-uses-date-prefix.md`, `manager-mispec-grep-c-for-occurrence-count.md`) will be deleted by Stage C's `rm -rf mistakes/*`. They served their iter1 purpose; trade-off is intentional.
- **H-3 (F-S-01 / F-U-01)** — Stage E split into E.1 (in-commit) and E.2 (post-commit terminal) with an explicit, concrete SHA gate.
...
- **M-1 (F-C-01)** — Success Criterion #2 rewritten: "exactly one new commit on `develop` post-merge (the squashed PR)" — the pre-merge sweep branch may carry multiple bisect-safe commits, which squash to one on develop.
- **M-2 (F-C-02)** — Stage G gains a post-merge step `git branch -d <sweep-branch>` so Success Criterion #5 holds.
- **L-1 (F-S-02)** — Stage F `find` updated to `find .gobbi/projects/gobbi/worktrees/ -mindepth 1 -type d -empty -delete` so the parent `worktrees/` dir survives.
- **M-3 (F-P-03)** — Stage E.1 explicitly names `2026-05-21-c676684d-...` in the delete-set; I5 acknowledges this is the prior session that promoted today's three mistakes (whose anchor frontmatter references will become dangling, accepted per H-2).
...
created-at: 2026-05-21T05:34Z (iter1) — 2026-05-21 iter2 revision
final-iter: iter2 (post Claude-evaluator REVISE remediation)
```
...
### Decisions Locked (15 total — unchanged from iter1)
...
- **Q-B resolves**: delete the bare-UUID dir with the other 52 in the sweep. **iter2 H-3 tightens the gate**: the bare-UUID delete is now Stage E.2 (terminal post-commit operation) with an explicit, testable SHA gate — runs only after the sweep commit's SHA exists in git AND has been written into `session.json`.
...
- iter2 H-2: the user explicitly accepts that the 3 promoted mistake files in `mistakes/` will be deleted with the rest of the subdir's content. They served their iter1 purpose (the iter2 evaluator findings already absorbed their lessons; this draft applies the `executor-rationalized…` lesson via the H-3 SHA gate and the `manager-mispec-grep-c` lesson via D2's command choices).
...
| S6 | Edge | (iter2 H-3) CLI's live bare-UUID session dir `6637e759-...` deletion mid-workflow could cause the CLI to lose track of the session. Resolved by Stage E.2: bare-UUID delete is a terminal post-commit operation gated by the sweep commit SHA being present in session.json. | Executor verifies the sweep commit SHA is in session.json before running the E.2 `rm -rf`. |
...
- [ ] **Commit checkpoint**: `git commit -m '<sweep-branch commit 3 msg>'`. Capture the resulting SHA — Stage E.2's gate depends on this SHA being present in `session.json`.

**Stage E — Session-dir sweep (Q8, Q-B) — SPLIT per iter2 H-3 into E.1 (in-commit) + E.2 (post-commit terminal)**
...
- [ ] Confirm THIS-workflow session-memory writes (Memorization promotion outputs, decisions log, final session.json — including the recorded sweep-branch commit SHA from Stage D) are present in the staged contents.
...
- [ ] If Stage D's gitignore edits and Stage E.1's `git add` of the kept session dir + `git rm` of any tracked session content were committed separately from Stage D (i.e., as their own bisect-safe commit), commit them now and re-record the SHA in session.json. Otherwise, the session-add is in Stage D's commit.

**Stage E.2 — TERMINAL post-commit operation (NOT part of any commit) — bare-UUID `6637e759-...` delete [iter2 H-3 SHA gate]**

- [ ] **Gate (concrete + testable)**: do NOT proceed until BOTH:
  1. The sweep-branch commit containing the gitignore edits + the staged session dir exists in git (verifiable: `git log --format=%H -1 <sweep-branch>` returns a SHA), AND
  2. That commit's SHA has been written into `.gobbi/projects/gobbi/sessions/2026-05-21-6637e759-84d9-403d-87bd-0a484abec245/session.json` (verifiable: `grep -F '<sha>' .gobbi/projects/gobbi/sessions/2026-05-21-6637e759-84d9-403d-87bd-0a484abec245/session.json` returns at least one match).
- [ ] If either condition fails, NEEDS_CONTEXT — do NOT rationalize the gate (per `executor-rationalized-failing-verification-gate.md`).
- [ ] When the gate passes: `rm -rf .gobbi/projects/gobbi/sessions/6637e759-84d9-403d-87bd-0a484abec245/`. This is a pure filesystem operation; it is NOT part of any commit (the dir was gitignored, so it was never in any tree).
...
**Stage G — PR open + post-merge local cleanup**

- [ ] Push the sweep branch.
- [ ] Open PR into `develop`. The tag `pre-reset-2026-05-21` already exists on origin (Stage 0).
- [ ] PR body cites the 15 locked decisions + the pre-reset tag + the iter2 remediation deltas.
- [ ] After PR review + approval: `gh pr merge --squash --delete-branch` (squashes the sweep-branch commits into one new commit on `develop`; deletes the remote sweep-branch).
...
**Critical ordering invariants (summary — refined per iter2 H-3):**
...
4. **Stage E.2 bare-UUID delete is TERMINAL POST-COMMIT, gated by the sweep commit SHA being recorded in session.json** (S6 + iter2 H-3). E.2 is FS-only; it is NEVER part of any commit.
...
### D1 — Deletion order across the 7 items + iter2 H-1 CLAUDE.md edit

Order: (0) tag → (A) pre-flight + branch open → (B) code/plugin/root + `.claude/project/gobbi/` + **CLAUDE.md lines 61-62 excision** → (C) adversarial-review + placeholder reset → (D) gitignore edits committed → (E.1) sessions sweep in-commit (52 dirs incl. `c676684d-`) → (E.2) bare-UUID FS-only delete gated by SHA-in-session.json → (F) worktree removal + branch deletion → (G) PR open + squash-merge + local sweep-branch delete.

Rationale anchored to I6 + I2 + I1b + Q-B + Q-F + iter2 H-1/H-3: the tag must point at the PRE-reset commit; `.gitignore` edits must be committed before `git add` on the kept session dir; the bare-UUID delete is terminal-post-commit (not part of any commit); CLAUDE.md's surgical 2-line excision lands in Stage B alongside the other Stage-B deletions; worktree removal must precede branch deletion; local sweep-branch deletion is the post-merge tail.

### D2 — Verification at the end (S1, S2, S3, S3b, S10, S13, S14)
...
15. `git ls-remote --tags origin | grep -c 'refs/tags/pre-reset-2026-05-21$'` → 1. (`$` anchor + line-form output makes `grep -c` a line-count safely usable here; for occurrence counts where lines might collide, use `grep -o ... | wc -l` per `manager-mispec-grep-c-for-occurrence-count.md`.)
16. **(iter2 H-1 / S3b)** `grep -nE '\[`v050-(overview|cli)\.md`\]' .claude/CLAUDE.md` → empty.
17. **(iter2 S14)** `test -d .gobbi/projects/gobbi/worktrees` → exit 0 (the `worktrees/` dir survives the `find -mindepth 1` cleanup).
18. **(iter2 M-1)** Post-merge: `git log --oneline -2 develop` shows exactly two commits, the latest being the squashed sweep PR and the prior being `487fc35`.
...
| D2 verification | 18 verification commands at PR ready-for-merge / post-merge |
...
| D9 SHA gate (iter2 H-3) | Concrete, testable: SHA exists in git AND in session.json before E.2 runs |
...
### D9 — Bare-UUID session-dir delete sequencing (Q-B + iter2 H-3 SHA gate)
...
**iter2 H-3 mitigation (concrete + testable, replacing iter1's vague "after the workflow's writes are committed")**:

- The bare-UUID delete is split out as Stage E.2 — a TERMINAL post-commit FS-only operation, not part of any commit.
- The gate has TWO conjunctive conditions:
  1. The sweep commit (the one containing the staged session dir + the gitignore edits) exists in git: `git log --format=%H -1 <sweep-branch>` returns a SHA.
  2. That SHA has been written into the surviving session's `session.json`: `grep -F '<sha>' .gobbi/projects/gobbi/sessions/2026-05-21-6637e759-.../session.json` returns ≥1 match.
- The executor MUST NOT proceed if either condition fails — return NEEDS_CONTEXT instead of rationalizing (per `executor-rationalized-failing-verification-gate.md`).
...
Verification: post-edit, `grep -nE '\[`v050-(overview|cli)\.md`\]' .claude/CLAUDE.md` returns empty. Pre-edit, the same command returns 2 hits (verified at iter2 authoring time).
...
| `.gobbi/projects/gobbi/mistakes/executor-rationalized-failing-verification-gate.md` | Drives the iter2 H-3 concrete SHA gate; will be deleted by Stage C per H-2 trade-off | Read |
...
| `.claude/CLAUDE.md` (lines 61-62) | iter2 H-1 verification: confirmed two design-link table rows exist; both will be removed | Read (iter2) |
...
### AskUserQuestion outcomes — 15 locked sub-decisions + 2 iter2-round answers

**Locked sub-decisions (15, rounds 1–3 — unchanged from iter1):**
...
**iter2 round (post-evaluator REVISE):**

1. **Survivor set expansion — Q-H equivalent**: User answer: **don't expand survivor set; fix the citations instead.**
   - Q-A stays as originally locked: survivor content is `agents/` + `skills/` + `rules/` only; `design/`, `mistakes/`, `backlogs/` and 10 other subdirs remain in PLACEHOLDER.
   - iter2 H-1 adds a new Stage B step editing `.claude/CLAUDE.md` to remove lines 61-62 (the two design link table rows). This lands in the sweep commit.
   - **Trade-off accepted (H-2)**: the 3 promoted mistake files in `.gobbi/projects/gobbi/mistakes/` (`executor-rationalized-failing-verification-gate.md`, `session-dir-naming-convention-uses-date-prefix.md`, `manager-mispec-grep-c-for-occurrence-count.md`) WILL BE DELETED by Stage C's `rm -rf mistakes/*`. They served their iter1 purpose: the iter2 evaluator findings already absorbed their lessons, and the iter2 draft applies those lessons (H-3 SHA gate from `executor-rationalized…`; D2 verification command choice from `manager-mispec-grep-c…`; M-3 explicit naming of `c676684d-...` from `session-dir-naming…`). The user explicitly accepts this trade-off.
   - **Backlog fate (H-4)**: the staged backlog at `staging/backlogs/project/cli-regenerates-gobbi-gitignore.md` has no project-level promotion target (Q-A places `backlogs/` in PLACEHOLDER). It remains session-scoped under the preserved session dir. Wrap-up will reference it in the handoff narrative but will NOT attempt project promotion.

2. **Stage E split — Q-I equivalent**: User answer: **split Stage E into E.1 (in-commit) and E.2 (post-commit terminal) with explicit SHA gate.**
   - E.1: in the sweep commit, delete the 52 legacy session dirs (`rm -rf` on the bare-UUID dirs + the prior date-prefixed `2026-05-21-c676684d-...` + the two fixture dirs). Stage the surviving `2026-05-21-6637e759-...` dir for tracking.
   - E.2: separate terminal FS-only operation (not part of any commit). Deletes `6637e759-...` only. Gate: the sweep commit's SHA must exist in git AND must be written into `session.json` before E.2 runs.
   - The gate is concrete + testable; encoded in Implementation Checklist → Stage E.2 and D9.

**Cross-decision mitigation note (Q-B + iter2 H-3):** the CLI's live bare-UUID session dir is deleted in this sweep. iter2 H-3 tightens the prior ambiguous "after the workflow's writes are committed" gate into a concrete two-condition SHA gate (see D9). Acceptable downside: the CLI may report "session not found" on next bootstrap — non-fatal, self-resolves.
...
See Memory reads register. Three project-level mistakes were directly load-bearing: `session-dir-naming-convention-uses-date-prefix` (I4 + Q8/Q-B interaction + M-3 explicit naming), `executor-rationalized-failing-verification-gate` (H-3 SHA gate design + E.2 NEEDS_CONTEXT discipline), `manager-mispec-grep-c-for-occurrence-count` (D2 #15 `$`-anchored `grep -c` audit). Per iter2 H-2, all three files are deleted by Stage C — their lessons are encoded in this draft and survive that deletion.

### Preserve list (iter1 evaluator must-preserves — confirmed retained in iter2)

Per `<session-dir>/ideation/evaluation/iter1/claude/overall.md` § "Preserve list (must not be broken by REVISE)", iter2 preserves all 9 items verbatim:
...
3. **Q-B mitigation: bare-UUID delete is LAST** — preserved AND tightened per F-S-01 / F-U-01 (Stage E.2 with concrete SHA gate).
4. **The 5 critical-ordering invariants** — preserved (now 6 invariants in the summary block); language refined per F-S-01.
5. **Memory reads register and Decisions Log traceability** — preserved + extended for iter2 reads.
6. **Mixed `git rm` vs `rm -rf` discipline per item** — preserved; inline commit-vs-FS labeling added.
7. **Out-of-Scope enumeration** — preserved; only delta is the H-1 narrow CLAUDE.md exception is noted.
8. **15 locked decisions enumerated in two tables (top + bottom)** — preserved.
9. **External research skip reason** — preserved.

### Deferred follow-ups (not in iter2 scope)
...
None remaining. All 15 sub-decisions + 2 iter2-round answers are locked; one backlog entry remains as a deferred follow-up (the CLI regenerator fix, scoped to the next session).

### WORK exit checklist (Ideation skill)

- [x] Root problem named (cumulative drift; concrete pre-rebuild reset).
- [x] Approach concrete enough to decompose (15 locked sub-decisions + 2 iter2 answers; ordered stages 0/A–G with E.1/E.2 split).
- [x] Constraints and trade-offs explicit (`-d` vs `-D`, gitignore ordering, bare-UUID SHA gate, regenerator follow-up risk, mistake-file deletion trade-off, backlog session-scoping).
- [x] Success measurable (18 verification commands in D2; 12 Scope-Contract success criteria).
- [x] Open questions: none. All iter2 evaluator findings (4 High + 4 Medium/Low) addressed.
- [x] Backlog stage covers all deferrals (1 backlog file for the CLI regenerator; explicitly session-scoped per H-4).
- [x] Memory reads register captures every load-bearing read (including iter2's evaluator file reads).
- [x] External research skip reason logged.
- [x] Steel-man checked and addressed (Q-F tag satisfies it).
- [x] Three mistakes (date-prefix, verification-honesty, grep-c) explicitly applied — even though the files themselves will be deleted per H-2, the lessons are encoded in the iter2 draft (M-3, H-3 gate, D2 #15 audit).
- [x] iter1 evaluator's must-preserve list (9 items) confirmed retained.
- [x] All 4 High findings addressed: F-P-01 (H-1), F-R-02 (H-2), F-S-01/F-U-01 (H-3), F-OV-01 (H-4).
- [x] All 4 Medium/Low surgical fixes addressed: F-C-01 (M-1 Success #2 rewrite), F-C-02 (M-2 post-merge branch -d), F-S-02 (L-1 -mindepth 1), F-P-03 (M-3 explicit c676684d naming).
