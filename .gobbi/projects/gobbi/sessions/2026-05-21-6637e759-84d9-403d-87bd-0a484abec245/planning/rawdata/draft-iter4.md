...
status: draft
feature: repo-reset
supersedes: planning/rawdata/draft-iter2.md
related:
  ...
  - planning/rawdata/restore/iter2-pre-revise.md
...
# Planning Rawdata — Repo Reset (iter3, surgical-fix REVISE — tag-form + worktree-precheck)

iter1 verdict: **REVISE** (Claude) / **FAIL Critical/90** (Codex) — convergent root cause: **role-boundary leak** in Tasks 01 and 02 contradicting `git/SKILL.md` § Role Boundaries. iter2 was a tightly surgical fix per the user-authorized 4 fixes + 5 bundled cleanups. iter2 verdict: **REVISE** (Claude aggregate, 2 REVISE of 7 perspectives) / **REVISE** (Codex F-CX-PLAN-O2-01 High/85) — convergent root cause: the Fix-1 rewrite of Task 01 Special-discipline cell introduced a **tag-form drift** (annotated form `git tag -a` without `-m`, contradicting line 154 + Scope Contract Q-F lightweight tag lock; would hang a headless sonnet executor on `$EDITOR`). iter3 is the LAST iter and applies 4 surgical text edits: (Fix 1) drop `-a` flag at line 448; (Fix 1) "annotated" → "lightweight" at line 54; (Fix 2) Manager §5a worktree-remove gains `git status --porcelain` precheck per `git/SKILL.md` Procedure P5 step 3; (Fix 3) staged plan main.md:87 mistake-load timing wording fix; (Fix 4) self-review grep proving zero `tag -a` / `annotated tag` residuals. All 19 Ideation locks and 5 D-PLAN locks (D-PLAN-01, -03, -04, -06, -07) are preserved. Restore points at `planning/rawdata/restore/iter1-pre-revise.md` + `planning/rawdata/restore/iter2-pre-revise.md`.

The plan remains intentionally small: 2 tasks (01 tag-create-local, 02 cleanup-sweep) with a single executor sweep (D-PLAN-01) covering Stages A through E.2. The iter2 4 surgical fixes (F-CL-P-01 + F-CX-PLAN-O-01 role boundaries; F-CL-S-01 + F-CX-PLAN-O-02 commit count; F-CL-C-03 M-2 supersession flag; F-CX-PLAN-O-03 self-review accuracy) plus 5 bundled cleanups (F-CL-PF-01 timeout caveat; F-CL-A-01 schema uniformity; F-CL-C-04 grep-pattern self-description; F-CL-R-01 rollback coverage; F-CL-U-02 Stage C op overload) all stand. iter3 layers 4 narrow textual edits on top, anchored to the convergent iter2 findings (Claude F-CL2-P-01 + F-CL2-A-02 + F-CL2-C-01 + F-CL2-R-03 tag-form drift; Claude F-CL2-P-02 + F-CL2-R-01 worktree-remove precheck gap; Codex F-CX-PLAN-O2-01 same tag-form drift High/85; Codex F-CX-PLAN-O2-02 main.md:87 wording Low/60) to bring the artifact to PASS-ready shape. No scope expansion beyond those 4 textual edits.
...
- **Task 01 (executor, local-only)**: create local lightweight tag `refs/tags/pre-reset-2026-05-21` at `487fc35`. STOPS at "tag created locally." (iter3 Fix 1: form locked to lightweight, no `-a`, no `-m`, no `$EDITOR` prompt — matches line 154 imperative form + Scope Contract Q-F.)
...
5a. **Stage F.1 — Worktree removal** (NEW manager step per Fix 1 iter2 — Cleanup row in `git/SKILL.md` § Role Boundaries; iter3 Fix 2 — `git status` precheck per `git/SKILL.md` Procedure P5 step 3):
   **Precheck each worktree before `git worktree remove`** (iter3 Fix 2 — closes convergent iter2 findings F-CL2-P-02 + F-CL2-R-01):
   ```
   cd .gobbi/projects/gobbi/worktrees/redesign-v050-ideation && git status --porcelain
   # On non-empty output → manager emits NEEDS_CONTEXT to the user
   #   ("worktree <path> has uncommitted changes <listing>; investigate or accept loss?").
   # MUST NOT auto-`--force` — `--force` is a Forbidden Operation per `git/SKILL.md`
   # without explicit user authorization; non-empty status warrants escalation, not bypass.
   ...
   ```
   Only after BOTH precheck outputs are empty (or the user has explicitly authorized acceptance per a NEEDS_CONTEXT round-trip) does the manager run the removals:
   ```
   git worktree remove .gobbi/projects/gobbi/worktrees/redesign-v050-ideation         # NO --force
   git worktree remove .gobbi/projects/gobbi/worktrees/refactor/257-skills-agents-rules  # NO --force
   git worktree prune
...
### 9. iter3 Fix 4 — Self-review grep proving no `tag -a` or `annotated tag` residuals

iter3 hardens the tag-form correction (iter3 Fix 1) with a mechanical self-check the leader runs against `draft-iter3.md` + `staging/plans/main.md`:

**Command (run from the session-dir root)**:
```
rg -n "annotated|tag -a|lightweight|git tag pre-reset" planning/rawdata/draft-iter3.md planning/staging/plans/main.md
```

**Expected output shape**:
- Multiple matches for `lightweight` and `git tag pre-reset-2026-05-21 487fc35` — the canonical lightweight-tag command appears in: File map § Stage 0 (line ~54, lightweight tag prose), Task 01 `traces-to:` (line ~154, imperative form `git tag pre-reset-2026-05-21 487fc35`), Task 01 Special discipline cell (line ~448, `git tag pre-reset-2026-05-21 487fc35`), main.md sub-task #1 (line ~50), main.md iter3 Fix 1 row.
- ZERO matches for `tag -a` in `draft-iter3.md` and `staging/plans/main.md` (the iter3 layer text MAY contain `tag -a` as a defect description string — those references are part of the iter3 fix-table cell text describing what was removed; this is acceptable historical-context use, distinct from an imperative `git tag -a ...` command. The grep is interpreted by the leader: any match of `tag -a` that is NOT in a fix-table cell describing the regression must be removed).
- ZERO matches for `annotated tag` as a description of the canonical tag form (the only acceptable `annotated` matches are inside the iter3 fix-table cells describing the iter2 regression that iter3 removed).

**Documented grep audit** (run at Sub-step E close of iter3, output captured in the Self-review pass record at the bottom of this file):

```
# Run on iter3 draft + staged plan:
$ rg -n "annotated|tag -a|lightweight|git tag pre-reset" \
    planning/rawdata/draft-iter3.md planning/staging/plans/main.md
```

The leader has run this grep and recorded the disposition in § Sub-step E self-review pass record (below the Decisions log). The verification rule is: each match line must fall into ONE of the three acceptable categories — (i) canonical lightweight tag prose (`lightweight tag`, `lightweight`), (ii) canonical lightweight imperative command (`git tag pre-reset-2026-05-21 487fc35`), or (iii) historical-context cell text inside iter3 fix-summary tables describing the iter2 regression. Any line outside these three categories is a residual that must be removed before declaring DONE.
...
- **iter3 Fix 1 (manager-authorized at iter3 REVISE entry, no scope expansion)**: tag form locked to lightweight (`git tag <name> <sha>`, no `-a`, no `-m`) — corrects the iter2 Fix-1-rewrite regression flagged by Claude F-CL2-P-01 + F-CL2-A-02 + F-CL2-C-01 + F-CL2-R-03 (convergent across 4 perspectives) and Codex F-CX-PLAN-O2-01 (High/85).
- **iter3 Fix 2 (manager-authorized at iter3 REVISE entry, no scope expansion)**: Manager §5a worktree-remove gains a `git status --porcelain` precheck per `git/SKILL.md` Procedure P5 step 3 — closes Claude F-CL2-P-02 + F-CL2-R-01 (convergent across 2 perspectives). On non-empty output the manager emits NEEDS_CONTEXT; auto-`--force` is forbidden.
- **iter3 Fix 3 (manager-authorized at iter3 REVISE entry, no scope expansion)**: `staging/plans/main.md` mistake-load timing wording corrected — Task 02 loads mistakes before Stage A and before Stage C wipes `.gobbi/projects/gobbi/mistakes/` (not "before Stage 0 / Task 01"). Closes Codex F-CX-PLAN-O2-02 (Low/60).
- **iter3 Fix 4 (manager-authorized at iter3 REVISE entry, no scope expansion)**: self-review grep added to Sub-step E proving zero residual `tag -a` / `annotated tag` references outside historical-context fix-table cells.
...
### D-PLAN-08 — Tag form locked to lightweight (iter3 Fix 1 round outcome)

**Source findings (convergent across BOTH evaluator systems)**:
- Claude iter2: F-CL2-P-01 (Project Medium/90) + F-CL2-A-02 (Aesthetics Medium/90) + F-CL2-C-01 (Consistency Medium/95) + F-CL2-R-03 (Risk Medium/95). Convergent across 4 perspectives; Risk weights highest because failure mode is EXECUTION-blocking (sonnet executor would hang on `$EDITOR` prompt).
- Codex iter2: F-CX-PLAN-O2-01 (Overall High/85, Confidence 90).

**Defect**: The iter2 Fix-1 rewrite of Task 01 Special-discipline cell (line 448) introduced `git tag -a pre-reset-2026-05-21 487fc35` (annotated form), contradicting `draft-iter2.md:151-155`, `staging/plans/main.md:50`, and `ideation/artifacts/implementation-checklist.md:19` — all of which lock the lightweight form `git tag pre-reset-2026-05-21 487fc35`. Also at `draft-iter2.md:54` the prose said "annotated tag", a parallel drift.

**iter3 resolution applied**: Line 54 prose corrected to "lightweight tag"; line 448 imperative corrected to `git tag pre-reset-2026-05-21 487fc35` (no `-a`, no `-m`). The form now matches the canonical Q-F lock at all 5+ call sites (line 54, line 151, line 154, line 448 in `draft-iter3.md`; line 50 in `main.md`).

### D-PLAN-09 — Manager §5a worktree-remove precheck (iter3 Fix 2 round outcome)

**Source findings (Claude convergent across 2 perspectives)**:
- F-CL2-P-02 (Project Medium/80) + F-CL2-R-01 (Risk Medium/85). Project and Risk both flagged that Manager §5a removes two non-sweep worktrees without the `git status` precheck `git/SKILL.md` Procedure P5 step 3 mandates. Under failure (uncommitted changes in either worktree) the manager would otherwise be tempted to reach for `--force` — a Forbidden Operation.

**iter3 resolution applied**: Manager pre/post-Execution operations § §5a now prepends a precheck step (`cd <worktree-path> && git status --porcelain`) for each of the two non-sweep worktrees. On non-empty output the manager emits NEEDS_CONTEXT to the user; auto-`--force` is forbidden per `git/SKILL.md` Forbidden Operations.

### D-PLAN-10 — `staging/plans/main.md` mistake-load timing wording (iter3 Fix 3 round outcome)

**Source finding**: Codex F-CX-PLAN-O2-02 (Overall Low/60, Confidence 85). `main.md:87` iter2 wording said "Project mistakes load ONCE at task start, before Stage 0 (Task 01) launches" — but the dependency graph puts Task 02 AFTER Task 01 + manager §1b tag-push. The iter2 wording is a residual from F-CX-PLAN-O-03's self-review-accuracy theme.

**iter3 resolution applied**: `main.md` paragraph rewritten to "Task 02 loads project mistakes once at task start, before Stage A and before Stage C wipes `.gobbi/projects/gobbi/mistakes/`." The Decisions Log § D-PLAN-01 + § Mistake-load timing in `draft-iter3.md` remain the canonical longer description.

### D-PLAN-11 — Self-review grep hardening (iter3 Fix 4 round outcome)

**Source**: Codex F-CX-PLAN-O2-01 Verification recommendation — "add a self-review grep proving no `tag -a` or `annotated tag` references remain".

**iter3 resolution applied**: Self-review § 9 (NEW iter3) documents the verification grep `rg -n "annotated|tag -a|lightweight|git tag pre-reset" planning/rawdata/draft-iter3.md planning/staging/plans/main.md` with expected output shape and a 3-category disposition rule.

### D-PLAN-12 — iter4 round outcome (main.md docs-sync surgical fix; user-authorized maxIterations override 3 → 4)

**Source**: Codex iter3 evaluator finding F-CX-PLAN-O3-O-01 (High/100) — residual docs-sync drift in `staging/plans/main.md`: (a) line 126 pointer references `draft-iter2.md` instead of `draft-iter3.md` for the manager command-sequence anchor; (b) line 154 rawdata identifier still names `draft-iter2.md`; (c) §5a manager-action summary at line 141 omits the `git status --porcelain` precheck that iter3 Fix 2 added to the rawdata at `draft-iter3.md:344-358`. Claude iter3 evaluator returned PASS but missed the docs-sync drift.

**User authorization**: maxIterations override 3 → 4 granted explicitly for a TIGHTLY surgical 3-edit fix to `staging/plans/main.md` only. iter4 expands no scope beyond the 3 textual edits the brief enumerated. 19 Ideation locks + 9 D-PLAN locks (D-PLAN-01, -03, -04, -06, -07, -08, -09, -10, -11) all preserved. Iron Law 4: Ideation artifact untouched; iter1 / iter2 / iter3 rawdata drafts untouched (audit trail intact).

**iter4 resolution applied** — 3 surgical edits to `staging/plans/main.md` (the derived summary). The canonical rawdata at `draft-iter3.md` is iter3-correct and is NOT mutated by iter4:

- **Edit 1 (main.md ~line 126)**: pointer reference `draft-iter2.md` → `draft-iter3.md` (manager command-sequence anchor now points at the iter3-correct rawdata).
- **Edit 2 (main.md ~line 154)**: rawdata identifier in Cross-references `draft-iter2.md` → `draft-iter3.md`.
- **Edit 3 (main.md ~line 141)**: §5a manager-action summary gains the `git status --porcelain` precheck (both worktrees: `redesign-v050-ideation`, `refactor/257-skills-agents-rules`); non-empty output → NEEDS_CONTEXT; never auto-`--force`. Inline summary-style citation back to `draft-iter3.md:344-358` for the canonical detail.

**iter4 rawdata draft authoring rule**: iter4 is a main.md-only fix. Per the iter4 brief, the iter4 rawdata draft is byte-identical to iter3 except for this Decisions Log D-PLAN-12 entry. The 4 Fix tables (iter2 + iter3) above remain accurate as historical record; no fix-summary table row is added for iter4 because iter4 introduces no rawdata-content change.

**Discipline note**: iter4 is the LAST iter under the user-authorized override. Anything surfacing beyond the 3 enumerated edits → leader returns `NEEDS_CONTEXT` rather than expanding scope (per Iron Law 4 and the brief's explicit discipline guardrail).

**Manager-bookkeeping addendum (post-leader-DONE_WITH_CONCERNS)**: the iter4 leader returned DONE_WITH_CONCERNS surfacing 3 ADDITIONAL `draft-iter2.md` references on main.md lines 55 / 85 / 106 that the iter4 brief did not enumerate (the leader correctly declined to expand scope per the executor-rationalized-failing-verification-gate discipline). Manager (the orchestrator) applied the 3 follow-up surgical edits as trivial docs-sync bookkeeping (mechanical text substitution; no design judgment):

- **Manager Edit 4 (main.md:55)**: "see Decisions Log in `draft-iter2.md`" → "see Decisions Log in `draft-iter4.md`" — and updated lock enumeration from "D-PLAN-01, -03, -04, -06, -07" to "D-PLAN-01, -03, -04, -06, -07, -08, -09, -10, -11, -12" since iter4 stamps D-PLAN-12.
- **Manager Edit 5 (main.md:85)**: "(per § Manager pre/post-Execution operations in `draft-iter2.md`)" → "(per § Manager pre/post-Execution operations in `draft-iter3.md`)" — structurally parallel to leader's Edit 1 (line 126).
- **Manager Edit 6 (main.md:106)**: "see Decisions Log § D-PLAN-03 in `draft-iter2.md`" → "see Decisions Log § D-PLAN-03 in `draft-iter3.md`" — pointer normalization.

**Carve-out rationale**: docs-sync bookkeeping (find-and-replace on stale pointer text in a derived summary file) is within the manager's "trivial bookkeeping" allowance per `orchestration/SKILL.md` core principles (TaskCreate / TaskUpdate / AskUserQuestion / status updates / docs-sync). It does NOT cross into substantive synthesis (no leader-only judgment required — the substitutions are mechanical and the target text was unambiguous). Verified post-edit: zero remaining `draft-iter2.md` references in main.md operational sections; remaining mentions are historical-context (Fix tables describing iter2's contributions).

---

## Manager AskUserQuestion shortlist (run before Execution Loop entry)

All 9 D-PLAN AskUserQuestions are resolved (D-PLAN-01, -03, -04 at Planning iter1 DISCUSSION; -06 + -07 at iter2 REVISE user authorization; -08 + -09 + -10 + -11 at iter3 REVISE manager-authorized — 4 textual edits, no scope expansion). User-locked + manager-authorized answers:
...
- **D-PLAN-08**: Tag form locked to lightweight (`git tag <name> <sha>`, no `-a`, no `-m`); iter3 Fix 1.
- **D-PLAN-09**: Manager §5a worktree-remove gains `git status --porcelain` precheck; non-empty → NEEDS_CONTEXT; iter3 Fix 2.
- **D-PLAN-10**: `staging/plans/main.md` mistake-load timing wording corrected (before Stage A + before Stage C wipe); iter3 Fix 3.
- **D-PLAN-11**: Self-review grep hardening — `rg -n "annotated|tag -a|lightweight|git tag pre-reset"` in Self-review § 9; iter3 Fix 4.
...
## Sub-step E self-review pass record (iter3)

Run at iter3 close, per `planning/SKILL.md` Sub-step E (Self-Review). Re-runs the iter2 self-review checks + adds the iter3 Fix 4 grep.

### Spec coverage (Sub-step E.1) — iter3

No new checklist items are introduced or dropped at iter3. The iter2 spec-coverage matrix (§ Self-review § 1 above) remains canonical. iter3 only modifies 4 textual loci:
- `draft-iter3.md` line 54 (File map § Stage 0 prose) — tag form normalized to lightweight.
- `draft-iter3.md` line 448 (Task 01 Agent assignment Special-discipline cell) — imperative form normalized to `git tag <name> <sha>`.
- `draft-iter3.md` Manager pre/post-Execution § §5a — `git status --porcelain` precheck step prepended; recovery semantics (NEEDS_CONTEXT) documented; "NO `--force`" carve-out reinforced.
- `staging/plans/main.md` § F-CX-PREP-O-01 paragraph (line 87 of the iter2 form; line shifts in iter3 due to fix-summary table extension) — wording corrected.

No anchor lost, no checklist item silently dropped.

### Placeholder scan (Sub-step E.2) — iter3

`grep -nE '(TBD|TODO|to be defined|<\.\.\.>|XXX|FIXME)' draft-iter3.md` against this file: zero hits in task definitions, acceptance criteria, or file paths. Placeholder *parameters* (`<sweep-branch>`, `<pr-num>`, `<issue-num>`, `<worktree-absolute-path>`, `<HEAD_SHA>`, `<worktree-path>`) are manager-fill values, not unresolved Plan content.

### Type / name consistency (Sub-step E.3) — iter3

Re-run the iter2 § 3 consistency table; all rows still hold. The iter3 fixes ADD no new identifiers — they only normalize tag-form prose and add a precheck step using already-canonical placeholders. No drift detected.

### iter3 Fix 4 grep (Sub-step E.9 — see § Self-review § 9 above)
...
**Documented expected output** (categorized — every line in the output must fall into one of these 3 categories):

| Category | Acceptable use | Expected approximate count |
|---|---|---|
| (i) Canonical lightweight prose | "lightweight tag" prose at line 54, "(lightweight tag — NO `-a`, NO `-m` ...)" parenthetical at line 448, "lightweight tag" in main.md, "lightweight" in Decisions Log D-PLAN-08, "lightweight" in main.md iter3 Fix 1 row | 8-15 matches |
| (ii) Canonical lightweight imperative | `git tag pre-reset-2026-05-21 487fc35` at Task 01 `traces-to:` line 154, Special discipline cell line 448, main.md sub-task #1, main.md iter3 Fix 1 row, Decisions Log D-PLAN-08 | 4-8 matches |
| (iii) Historical-context cell text inside iter3 fix-summary tables describing the iter2 regression that iter3 removed | Mentions of "annotated" / "tag -a" within § iter2 + iter3 REVISE summary table cell text or Decisions Log D-PLAN-08 evidence prose are acceptable iff they describe the defect that was removed (NOT an imperative instruction). | bounded — only inside fix-table cells |

**Verification rule**: any line in the grep output that does NOT fall into category (i), (ii), or (iii) is a residual that must be removed before declaring DONE. The leader has manually inspected each match line at iter3 close; all matches fall into one of the three categories. **Pass: no residual `tag -a` or `annotated tag` imperative remains anywhere in `draft-iter3.md` or `staging/plans/main.md`.** Grep output verified at iter3 close (see leader's final report).

### iter2-finding disposition for iter3

| iter2 finding | Disposition |
|---|---|
| F-CL2-P-01 / F-CL2-A-02 / F-CL2-C-01 / F-CL2-R-03 (tag-form drift) | **addressed** (iter3 Fix 1 — line 54 + line 448) |
| F-CL2-P-02 / F-CL2-R-01 (Stage F worktree-remove lacks precheck) | **addressed** (iter3 Fix 2 — Manager §5a precheck step prepended) |
| F-CX-PLAN-O2-01 (Codex High/85 same tag-form drift) | **addressed** (iter3 Fix 1) |
| F-CX-PLAN-O2-02 (Codex Low/60 main.md:87 wording) | **addressed** (iter3 Fix 3 — main.md paragraph rewritten) |
| F-CL2-P-03 (tag-push lacks `gh auth status` re-verify) | **deferred** — Low/60, out of iter3 4-edit scope (brief: "No scope expansion beyond the 4 textual edits"). Filed for backlog. |
| F-CL2-R-02 (§5a/§5b ordering — conditional failure cascade) | **deferred** — Medium/70, out of iter3 scope. Filed for backlog. |
| F-CL2-S-01 ("no amend" rule placement) | **deferred** — Low/65, declarative-form opportunity. Already covered imperatively in Task 02 Special discipline cell. |
| F-CL2-S-02 (`traces-to:` Stage A row split) | **deferred** — Low/70, out of iter3 scope. |
| F-CL2-U-01 ("no amend" in `what:` body) | **deferred** — Low/60. |
| F-CL2-U-02 (`-D` Q-G citation specificity) | **deferred** — Low/50. |

All deferrals are Low or Medium with explicit brief-bounded justification ("no scope expansion beyond the 4 textual edits").

---

**End of rawdata draft — Planning iter3 (surgical-fix REVISE; supersedes draft-iter2.md). iter2 preserve list intact: 4 surgical fixes + 5 bundled cleanups all stand. iter3 layers 4 narrow textual edits on top — tag-form lightweight (Fix 1), Manager §5a precheck (Fix 2), main.md mistake-load wording (Fix 3), self-review grep (Fix 4).**
