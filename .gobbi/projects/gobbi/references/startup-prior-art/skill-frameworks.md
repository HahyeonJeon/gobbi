...
- **Insight** — gstack writes ONE design doc per feature; gobbi synthesizes *atomic typed records* (finer granularity, better for supersession). But the template's discipline — every section traces to a specific interview answer, and the doc "stands on its own for a reader who never saw the talk" — is exactly gobbi's "A transcript is not memory" principle. gstack also carries a `Supersedes:` field creating a revision chain (gobbi does this via frontmatter supersession).
- **Why** — Reinforces gobbi's synthesis-not-transcript rule and its answer→record traceability; the section list is a checklist of what a complete baseline must cover.
...
- **Insight** — gobbi walks problem→boundary→solution by tree order and runs a contradiction pass + jargon-free challenge at P5, but it lacks an **explicit mid-conversation premise gate** ("here are the premises I now hold — agree/disagree?") *before crossing from problem space into solution space*. gstack makes premise-confirmation a hard turn.
- **Why** — A premise gate at the Phase-II→Phase-III boundary (before Topic 5 features) would catch a wrong framing before any product-shape work is done — cheaper than catching it in the P5 contradiction pass.
...
- **Why** — Directly maps onto gobbi Principle 3 ("design with the user, based on references") and the startup P3 micro-loop; the three-layer structure and the minimal-vs-ideal equal-weight rule are concrete upgrades to the "study prior art" step.
...
- **Why** — gobbi's `evaluation.md`/`checklist.md` bundle for the baseline can adopt these as explicit PASS predicates for the problem/users/value topics.
...
- **Insight** — gobbi's startup deliberately stops at DIRECTION (no mechanism), so the *spec-level* concreteness bar applies to gobbi's Ideation/Planning, not startup. BUT the **"read the repo first, show the verified fact, then ask"** rule is already gobbi startup doctrine (`topics.md`: "show any verified repo fact first to shorten the answer"; P2 "separate verified repo facts from claims"). gstack's contribution is naming it the *magical moment* and making it a HARD gate before any question in a code-touching branch.
...
12. **Independence at review: the reviewer cannot see the elicitation conversation; review on a fixed rubric (Completeness/Consistency/Clarity/Scope/Feasibility) with a convergence guard.** — gobbi's dual-system P6.5 gate is the same idea; the rubric + convergence guard are reusable, and gobbi's non-skippable stance is stronger than gstack's "bonus, not a gate." `[gstack:office-hours Spec Review Loop]`
13. **The synthesis must stand alone for a reader who never saw the talk; every section traces to a specific answer.** — "A transcript is not memory" (gobbi) = gstack's design-doc-with-section-per-answer + `Supersedes:` revision chain. `[gstack:design-and-handoff]` `[gobbi:startup SKILL.md]`
...
- **Three-layer prior-art study** (→ P3 micro-loop PRIOR-ART STUDY step): Layer 1 tried-true (don't reinvent) / Layer 2 new-popular (scrutinize) / Layer 3 first-principles (prize); log a "eureka" when Layer-3 contradicts convention. `[gstack:ETHOS.md]` `[gstack:office-hours Phase 2.75]`
- **Alternatives rule** (→ P3 RECOMMENDATION step): "≥2 options; one MUST be minimal-viable, one MUST be ideal-architecture, at equal weight; label each with effort (S/M/L/XL) + risk + what it reuses; recommend one; STOP for the user's decision." `[gstack:plan-ceo-review 0C-bis]`
...
- **"The Assignment" closure** (→ P7 close / open-questions): end with one concrete next action, each open question owning a resolution method. `[gstack:office-hours]`
