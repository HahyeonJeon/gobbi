---
name: plugin-hooks-config-and-plugin-root-var
...
Resolves the hook-portability question and makes "ship the working hooks inside the plugin" a low-risk,
body-unchanged relocation — the `${CLAUDE_PLUGIN_ROOT}` substitution is the only required change.
...
"${CLAUDE_PLUGIN_ROOT}: the absolute path to your plugin's installation directory. Use this to reference
