...
- **Option A (replace):** Leader recommendation. Cleaner single-source. Rejected because it breaks in-repo dev sessions (hooks stop firing unless the plugin is installed).
- **Option B (coexist):** Double-fire with no structural separation. Rejected as the worst of both worlds.
