...
description: Materialized-copy drift surface documented but no mechanical sync/diff gate or named trigger event at Ideation close; resolved in Preparation
type: decisions
scope: feature
...
**OPEN at Ideation close — RESOLVED in Preparation.** See `decisions/drift-resync-trigger-and-mechanical-gate-resolved.md` for the resolution.

Resolution summary: named trigger = any commit touching `.gobbi/projects/gobbi/skills/`, `.gobbi/projects/gobbi/agents/*.md`, or `.claude/hooks/*.sh` requires re-materializing `plugins/gobbi/{skills,agents,hooks}/` in the same commit; mechanical gate = `scripts/sync-plugin-package.sh` with `--check` diff/freshness mode.
