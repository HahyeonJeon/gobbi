...
The draft is self-mitigating: it acknowledges O-1 "was not a separately-enumerated gap" and explains why it is a leader classification, not a true staging gap. The `scenario_gap` finding PROJ-1 IS properly staged at `scenarios/process/8-seed-depth-parity-risk.md`. No actual staging miss exists. The error is an over-attribution of user lock to an item the user never saw named.
