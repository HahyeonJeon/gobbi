...
| 2026-06-25 | 463a1c96-f75c-4a14-80b4-f4d6815679cd | Memory-COMPACTION mechanism added (Wrap-up Stage-2c, ships DORMANT `enabled: false`): per-`{type}/{area}/` cap (softCap 12 / hardCap 15), uniform lossless merge→Map-of-Content + `git mv` to `archive/` (`archive_reason: merged`), new `check-merge-ref-integrity.sh` two-family gate, `validate-frontmatter` accepts `supersedes` list form; runs inside the non-skippable Stage-3 gate. Added 4 compaction prior-art references. Cap-tuning deferred (`archive/backlogs/memory/2026-07-20-compaction-cap-tuning.md`) |
