...
**In-scope (three tiers, priority order):**
...
**Out-of-scope:** re-home, big-bang rewrite, new eval perspective, Principle-13 surgery,
...
- **Edge (legitimate-key backlog):** a `backlogs/` doc with both legitimate `disposition` AND
  illegitimate eval-routing keys — FIX-1 predicate strips the eval keys, preserves `disposition`.
...
- Optimizes for: durable, machine-addressable project memory that a zero-context reader can
  navigate without re-reading session transcripts.
- Sacrifices: one-shot speed (wave-based is slower than big-bang, but de-risked per Principle 3).
- Wave-based approach honors the steel-man's anti-big-bang warning while staying bottom-up.
