...
- `scenarios/` — workflow/memorization.md doc-filename rename edge case; + iteration-artifact-snapshot-frozen-not-mutated (session `babc6f3b`)
...
| Date | Session | What |
|---|---|---|
| 2026-07-24 | 847bafc9-9659-46b4-b23e-653e25f0e9f5 | Planning-skill split session — **memory-only landing on develop**; the split itself is deferred as **draft PR #362**, which collides with the 2026-07-19 absorb-preparation redesign (develop made `planning/SKILL.md` a `skill-type: operation` skill; the branch made it a workflow-agnostic generic SOP + folded the Gobbi loop into `orchestration/workflow/planning.md`). Promoted 67 durable session-record files: 16 project mistakes (highest-value — self-verification-machinery + refactor-sweep + resilience traps), 12 planning-prior-art references, 14 checklists, 3 scenarios, 5 feature decisions, 8 Codex-waiver discussions, 8 backlogs, 1 journal note. Full workflow ran (Ideation/Preparation/Planning/Execution/Wrap-up all PASS; Wrap-up iter1 REVISE → iter2 PASS); Codex user-waived from Planning iter 4 (Claude-side classifier outage) → single-system Claude thereafter (`production_mode: single`, not degraded); cold-load P10 = COLD_LOAD_PASS. The generic-SOP split (9 commits `57f4b5d2`…`30f7bc39`, +1553/-999, pushed on branch `claude-2026-07-16-847bafc9…`) awaits design reconciliation vs develop's operation-skill planning. |
...
- `archive/backlogs/workflow/2026-07-20-persist-session-memory-past-cleanup.md` (project-level): retain session working memory after worktree cleanup for post-session debugging.
...
- `archive/backlogs/workflow/2026-07-20-project-skill-template-realign.md` (project-level, session `33de02b8`): RESOLVED — the project-skill template was relocated to `skill-writing/templates/project-skill.md` and realigned to the 6-section standard; backlog closed.
