...
## Suggested approach
Run a fresh Ideation Loop focused on full dual-system architecture. Start by reading `codex/delegation.md`, `orchestration/workflow/production.md`, `orchestration/workflow/evaluation.md`, `delegation/SKILL.md`, and the Codex bridge mistake records. Separate semantic orchestration from command-line wrapper mechanics so the new work does not duplicate the bridge child doc.
