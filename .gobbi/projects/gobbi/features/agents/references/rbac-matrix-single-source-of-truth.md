...
- `langgraph-skill-catalog-pattern` — the companion external-prior-art reference for the same role × phase skill-matrix design (skills-as-data catalog the matrix shape draws from). The frontmatter `related:` list is empty because no internal learning/mistake slug was tied to this reference; this is the one cross-reference that applies.
