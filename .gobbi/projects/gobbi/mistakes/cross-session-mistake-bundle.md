# Cross-session untracked mistakes need a worktree-PR bundle plan
...
When a session ends with an authored-but-uncommitted mistake file, the next session must:
...
4. Commit it as part of that PR's branch with a message like `docs(mistakes): bundle <slug> from prior session`
...
Authored mistakes are the highest-value knowledge in the system; leaving them un-promoted because of session-boundary friction defeats the whole point of the mistake discipline. The bundle pattern adds zero overhead (one extra commit on a worktree-PR that was going to ship anyway) and preserves the harness invariant that all develop changes go through PR review.
