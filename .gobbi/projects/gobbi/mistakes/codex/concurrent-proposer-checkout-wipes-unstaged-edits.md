...
# A peer Git cleanup can erase the active writer's worktree edits
...
## Correct approach
Keep one ordered worktree writer. Run opposite-system operations as ephemeral read-only processes and let only an active-runtime assistant store their validated artifact responses. A peer must never run Git cleanup or write generated files.
