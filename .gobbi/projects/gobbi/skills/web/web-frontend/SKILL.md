...
- **MUST scaffold the complete frontend and prove one real browser path to a result from its named source of
  truth before breadth.** A mock, screenshot, or polished placeholder is not integration evidence.
- **MUST keep trust, status, and evidence claims accurate.** Separate specification, implementation,
...
- Start with the scoped change outcome, current product, governing records, and available user evidence.
...
- Record the affected people and contexts, observed current product behavior, compatibility obligations, known
...
- Define every applicable normal, waiting, empty, partial, stale, degraded, invalid, unauthorized,
  ...
  including its transition and confirmed completion evidence.
...
- Specify its purpose, preconditions, inputs, outputs, semantics, accessible name, focus, feedback, error,
  recovery, responsive and localized behavior, trust impact, and evidence, citing borrowed claims beside the
  affected clause.
...
- Repair a missing clause, structural boundary, state owner, or simulation boundary before continuing; proceed
...
- Select the shortest complete path and order its work from native semantics, content, and local state through
  interaction, request and response, confirmed backend result, accurate completion, diagnostics,
  failure, and recovery.
- Implement the path through real integration points, preserving entered data and safe resumption where applicable and
  distinguishing disabled, pending, cancelled, failed, recovered, and completed states for sighted and
  non-sighted users.
- Verify the path in a live browser, including its real confirmed result at the named owner; do not treat a
  mocked backend, static capture, or polished placeholder as proof of integration, semantics, focus, or
  recovery.
- Add one complete visual, responsive, modality, localization, lifecycle, failure, or recovery slice at a
  time; repair false success, duplicate submission, lost state, inaccessible behavior, or missing accurate
...
- Ask `web-testing` to run suite evidence; inspect live keyboard, pointer, touch, and applicable
  assistive-technology interaction, DOM, accessibility trees, rendered states, and confirmed completion
  at their owners.
