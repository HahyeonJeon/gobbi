...
|---|---|---|---|
| `pi` | "What to do" — ideation, review, creative assessment | Ideation step and Evaluation step. Spawned in parallel with innovative + best stances. | Opus |
...
| `agent-evaluator` | Structured assessment — evaluates agent output against criteria | After creative or execution steps when evaluation is requested. | Sonnet |
...
During the Ideation step's investigation phase, the orchestrator spawns `researcher` agents to investigate "how to do" before execution begins. Investigation happens as an internal loop within Ideation — after the approach is concrete enough to plan against, and before the plan is approved for execution. The goal is to give executors direction and references so they can implement with confidence.
...
- Context to load — project skill, `gotcha`, `research`, domain skills relevant to the investigation
