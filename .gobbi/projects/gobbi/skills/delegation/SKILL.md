...
> **The workflow todo list is manager-owned.** The 6-step harness spine ([`orchestration/SKILL.md` § Harness Todo List](../orchestration/workflow/status-display.md#harness-todo-list)) is created and updated only by the manager; no `leader` / `executor` / `assistant` / `evaluator` subagent creates or updates it.
