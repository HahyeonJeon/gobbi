...
package contains both plugin manifests and generator-materialized `skills/` and `agents/` directories whose
files must be byte-equal to their canonical owners. Regenerate both components with
...
incomplete installed component fails. A missing installed path is a package failure.
