...
- Is the gotcha scoped to the right level — project-specific in project files, cross-cutting in `gotcha/`, skill-specific in the skill's own `gotchas.md`?
...
- Is the entry in the correct file — skill-specific gotchas in `{skill-name}/gotchas.md`, cross-cutting in `gotcha/{topic}.md`, project-specific in `$CLAUDE_PROJECT_DIR/.claude/project/{project-name}/gotchas/{category}.md`?
...
- `[semantic]` Scope matches file location — project-specific entries in project files, cross-cutting entries in `gotcha/`, skill entries in `{skill-name}/gotchas.md`
