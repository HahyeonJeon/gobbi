...
Project-specific gotchas capture mistakes that only matter in one project's context. They live alongside the project's documentation, not in the shared `gotcha/` skill files.
...
The test is confusion, not domain. If recording a mistake in `gotcha/` would cause agents on unrelated projects to second-guess a correct approach, it belongs in the project's own gotcha files instead. Project gotchas are scoped to one project; they cannot produce false positives elsewhere.
...
The `{category}` groups related mistakes — use the same categories as the central `gotcha/` files when the domain matches, or invent a category when the mistake is unique to the project. Read the existing project directory for context before choosing a category name.
...
**Record in `gotcha/{skill}.md`** when:
