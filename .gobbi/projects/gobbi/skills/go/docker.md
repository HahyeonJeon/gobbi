...
| §12 | `go fix` — available only on an **installed toolchain 1.26 or newer**, not gated on the module floor ([`modules-tooling.md`](modules-tooling.md) §7 owns it) — clears three of the four deprecated client symbols, not four. `NewClientWithOpts` → `New`, `WithVersion` → `WithAPIVersion`, and `WithVersionFromEnv` → `WithAPIVersionFromEnv` each carry `//go:fix inline`. **`WithAPIVersionNegotiation` carries no `//go:fix` directive** — it is deprecated and its body returns `nil`, so deleting the call is a hand edit the fixer will not make for you | `client/client.go` line 170, `client/client_options.go` lines 341 and 376, read 2026-07-26 |
