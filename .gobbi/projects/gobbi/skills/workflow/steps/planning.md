...
Run [`dual-system-work.md`](dual-system-work.md) with the leader role and the same locked planning contract for both systems. The synthesis must contain a complete ordered plan, dependencies, write-conflict resolution, skill loads, scope boundaries, and verification criteria.

## EVALUATION

Run [`evaluation.md`](evaluation.md). Both fresh evaluators receive the locked Ideation evidence, complete Planning creation package, resolved decisions, coverage obligations, and canonical plan. Obtain the user's finding-disposition decision before RECORD.

## RECORD and exit

Run [`record.md`](record.md). On PASS, verify the canonical plan and task identities. Supply the complete task list to the record command's `scaffold-tasks`, run `verify` with that list, and transition to the first Execution task's DISCUSSION. On REVISE, FAIL, or a return to Ideation, follow [`state-machine.md`](state-machine.md). Empty staging is valid when no evidence supports a candidate.
