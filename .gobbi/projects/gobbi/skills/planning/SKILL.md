...
| 2 | Leader | Task + agent type | Propose **model override** if any. Defaults follow [delegation/SKILL.md § Model Selection](../delegation/SKILL.md#model-selection): executor→sonnet, leader→opus, assistant→sonnet. Override only when a task has unusual complexity or simplicity | Model override (if any) per task |
