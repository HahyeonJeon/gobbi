...
**What happened** — A Planning task plan wrote each task's `verifies:` gate as runnable shell intended to exit 0 AS WRITTEN in the plan. Two failures followed, surviving two REVISE iterations: (1) the gates ran the whole-file `scripts/check-markdown-links.sh` over target files that carry pre-existing, out-of-scope broken links, so the gate exits 1 regardless of whether the edit is correct — it can NEVER pass; (2) shell-mechanic bugs — a `! grep …` drift check is inert under `set -e` (the negation's exit is ignored), and the cross-file contract block was unreachable (chained after a failing link check). The leader tried to fix it by patching the shell twice; both REVISEs failed.
