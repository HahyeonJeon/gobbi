...
Load `claude` for the general documentation writing standard before authoring project docs.
...
**gotchas/** — Project-specific gotchas, categorized by domain. Mistakes and corrections that apply only to this project. Separate from cross-project gotchas in `gotcha/`.

**note/** — Workflow notes per task, managed by `note`. Each task gets a directory named `{YYYYMMDD}-{HHMM}-{slug}-{session_id}`.
