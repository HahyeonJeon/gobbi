...
Does the project doc compose with gobbi's conventions? Project-specific gotchas belong in `$CLAUDE_PROJECT_DIR/.claude/project/{project-name}/gotchas/`, not in the cross-project `gotcha` skill. Project-specific rules belong in `$CLAUDE_PROJECT_DIR/.claude/project/{project-name}/rules/`, not in `.claude/rules/`.
...
- `[structural]` Project-specific gotchas live in the project gotchas directory, not in `gotcha`
