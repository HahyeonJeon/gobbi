...
- **Declared scope + promote-up.** Each type declares its scope:
  ...
  - **Both** types: `decisions`, `design`, `mistakes`, `backlogs`, `references`. They default to feature-level and **promote up** to the project root only when the content sets a project-wide convention / cross-feature architecture (user-confirmed through the active runtime's user-decision primitive at Wrap-up).
