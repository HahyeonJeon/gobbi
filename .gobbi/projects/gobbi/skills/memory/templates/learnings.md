...
1. **Loop MEMORIZATION** (`ideation` / `planning` / `execution`): stage at `sessions/{date}-{session-id}/{N}-{loop}/staging/learnings/{slug}.md`. Loop MEMORIZATION **never** writes directly to project memory.
