...
- **Worktree creation.** `worktree-pr` invokes [`git/SKILL.md` § P2](../git/SKILL.md#p2----create-worktree) at row 5.5 to create a per-session worktree under `.gobbi/projects/<name>/worktrees/`. `direct` skips P2 entirely — the session works in the main tree at its current checkout.
...
Also verify `jq -r '.git.worktreePath'` returns a non-null value for `worktree-pr` sessions. A `null` `worktreePath` on a `worktree-pr` session indicates row 5.5 was skipped or P2 failed without surfacing an error.
