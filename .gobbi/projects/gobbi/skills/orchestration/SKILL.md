...
**Procedure.** Every session creates a worktree first, bootstraps the full record skeleton with `init-record-map.sh`, then resolves settings and stamps `state.json` and `session.json` inside that worktree.
...
| 4 | Init state.json | <ul><li>Stamp the row-2 `state.json` stub (already placed from `templates/state.template.json`) at `…/sessions/{date}-{session-id}/state.json`, rooted at the row-1 worktree path (in-turn value — `session.json` is not written yet).</li><li>Set `mode` from the resolved settings.</li><li>Mark `workflow.configuration.state = "Done"` and `workflow.ideation.state = "Active"` (Step 1 has just completed).</li></ul> | [state.template.json](templates/state.template.json) | manager |
