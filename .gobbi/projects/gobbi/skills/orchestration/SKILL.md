...
| `RECORD` | `EVALUATION` complete OR skipped per policy | `assistant` subagent | Write session staging for this iteration; memory promotion only in Wrap-up | Session record updated / iteration staged |
...
- **`REVISE` before the `maxIterations`-th WORK pass** (`iter + 1 < maxIterations`) → increment `iter`, attach the eval findings to the next delegation prompt, re-enter `DISCUSSION`. Re-entry is always at `DISCUSSION` — never directly at `WORK`. (At `maxIterations: 1` this branch never runs — one WORK pass is the whole budget.)
- **`REVISE` on the `maxIterations`-th WORK pass** (`iter + 1 == maxIterations`) → the budget is exhausted; exit to the mode's cap-exhaustion path. The failure is captured in this iteration's `RECORD`; the next loop's input notes the exhaustion. (Auto records the abort and continues per [`auto-mode.md §6`](auto-mode.md); Chat escalates to the user per [`chat-mode.md §5`](chat-mode.md).)
- **`FAIL`** (any evaluator `FAIL`) → **safety-gate escalation to the user** through the active runtime's user-decision primitive (the user decides revise / abort / re-enter the prior step). A `FAIL` is **never** auto-re-entered as a `REVISE` on either mode, and does not silently consume the iteration budget — it fires on any `FAIL` verdict, independent of the budget. This aligns with the per-loop `ITER / EXIT` tables and [`workflow/evaluation.md`](workflow/evaluation.md).
