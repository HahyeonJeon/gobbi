Sub-document of the `orchestration` skill. Used during the Ideation phase's investigation loop. Researcher agents investigate the best approach for each plan task before Execution begins, giving executors direction and references so they can implement with confidence.
...
Research is the investigation loop inside the Ideation phase — used during Ideation before the approach is locked for planning.
