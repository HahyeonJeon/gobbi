...
4. Constructs the assistant delegation prompt per [delegation prompt requirements](../delegation.md#what-every-delegation-prompt-contains).
...
- **Output path**: per-iter scoped under `sessions/{date}-{session-id}/4-wrap-up/evaluation/iter{n}/{system}/` — nine files per system: `{perspective}.md` (seven), `overall.md`, and the filled `checklist.md`
...
Wrap-up's loop dir is `4-wrap-up/`. Loop-specific files: WORK draft `working/draft-iter{n}.md`
