Sub-document of the `orchestration` skill. Used at the Memorization state within each workflow loop. An `assistant` agent reads the session so far, extracts decisions, state, open questions, and gotchas, and writes them where the next session can find them.
...
Write to `$CLAUDE_PROJECT_DIR/.claude/project/{project-name}/gotchas/`. Follow the `project-gotcha.md` child doc of gotcha for format and categorization.
...
Write to `$CLAUDE_PROJECT_DIR/.claude/project/{project-name}/rules/`. Follow the rules-doc skill for format and structure.
