Sub-document of the `orchestration` skill. Used at every workflow phase where the manager needs to clarify, challenge, or specify the user's intent before delegating work. Discussion resolves specification gaps — it turns ambiguous prompts into concrete, delegatable briefs.

---

## Core Principles

> **Be critical, not agreeable. The manager's job is to find problems before they become implementation mistakes.**
...
> **Discussion resolves specification gaps — "what do you want?" when the agent lacks information to act. Contribution points (ideation) resolve judgment gaps — "which decisions are yours to make?" when the user's domain knowledge would produce better outcomes than agent discretion. Different problems, different tools.**
