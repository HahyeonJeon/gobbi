...
|---|---|---|---|
| **Configuration (Step 1)** | The full session-record skeleton via `init-record-map.sh` (row 2: `transcripts/` + metadata stubs + the four loop dirs), then stamped `settings.json` (row 3) / `state.json` (row 4 fresh, or rehydrated at row 4R on resume) / `session.json` (row 5) | manager | Rooted at the row-1 worktree path; the initializer rejects legacy schema metadata before mutation. `session.json` row 5 stamps `git.worktreePath`, the durable write-root. See [§ Step 1 — Workflow Configuration](../SKILL.md#step-1--workflow-configuration). |
| **Loop entry (each of Steps 2-5)** | The loop's `{working, staging, evaluation}` subdirs (Execution: per-task `task-{NN}-{slug}/` quartets) | manager | Invokes [`scaffold-session-dir.sh`](../scripts/scaffold-session-dir.sh) `<session-root> <step-dir>` at loop entry to materialize the 4-slot interior, so WORK / EVALUATION / RECORD can assume the tree exists. See [§ Loop-entry scaffold](#loop-entry-scaffold). |
...
| **Wrap-up (Step 5)** | `staging/` trees promoted to memory; non-canonical session subdirs normalized going-forward | `assistant` (Wrap-up) | The only workflow-loop step that writes memory. Deviations from the canonical shape below are normalized here — see [`wrap-up/promotion.md` § Non-standard session-subdir cleanup](../../wrap-up/promotion.md#non-standard-session-subdir-cleanup-going-forward). |

**On-disk inventory.** The canonical shape the lifecycle above writes into is defined once, in [`record/record-map.md`](../../record/record-map.md) — the single source of truth for the per-session working tree. That doc carries the complete ASCII tree (session root + `{N}-{loop}/` ordinal map + the 4-slot loop interior `working/ evaluation/ staging/ outputs/` — `working/` itself carries the `research/` and `proposals/codex/` sub-slots (the latter holds the dual-system Codex proposer's frozen proposal, NOT a 5th top-level slot) — + the `3-execution/task-{NN}-{slug}/` nesting), the SEAM-3 rule (on-disk dirs carry the `{N}-` prefix; `workflow.{loop}` JSON keys stay bare), the transcript rules, and the path-validation contract. The prose in this skill points there rather than re-declaring the shape — a second copy is exactly the drift the spec doc exists to remove.
...
At each loop entry (Steps 2-5), the manager materializes the loop's interior by invoking the scaffold script rather than creating dirs ad hoc:
...
- `<step-dir>` — one of `1-ideation` `2-planning` `3-execution` `4-wrap-up`, or a single execution task dir `3-execution/task-{NN}-{slug}` (`{NN}` is `[0-9]{2}`, `{slug}` matches `[a-z0-9-]{1,40}`).
...
The Execution loop is per-task. Each task lives under `3-execution/task-{NN}-{slug}/` and carries the **full quartet** — `{working, staging, evaluation, outputs}`:
