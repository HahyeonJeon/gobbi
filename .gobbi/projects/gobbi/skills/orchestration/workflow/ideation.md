...
The Ideation Loop runs the four-phase iteration shape — `DISCUSSION` → `WORK` → `EVALUATION` → `RECORD` → `ITER / EXIT`. Phase semantics for Ideation:
...
| `WORK` | Leader documents the DISCUSSION outcome into the canonical working draft and stages reference + backlog artifacts under `sessions/{date}-{session-id}/1-ideation/staging/`. Documentation + session-record staging — no new content. |
...
| `RECORD` | Assistant runs **after every EVALUATION** (PASS or REVISE) to preserve the iteration's transcript and update `session.json`. On `PASS` it additionally emits the loop's `outputs/` files and stages typed-finding artifacts under `staging/`. **No writes to memory** — Wrap-up handles session → project promotion. |
...
**Manager's job**: spawn the leader for documentation + session-record staging. The leader's job in WORK is to record what was decided in DISCUSSION into a draft at `sessions/{date}-{session-id}/1-ideation/working/draft-iter{n}.md` and to stage reference + backlog artifacts under `sessions/{date}-{session-id}/1-ideation/staging/`.
...
- **Cross-system divergence** is derived by comparing per-system files at RECORD; no separate divergence file is written

Verdict is `PASS` or `REVISE`. **Both verdicts advance to RECORD first** (so each iteration's transcript + `session.json` entry is preserved regardless of outcome). The ITER / EXIT decision happens after RECORD.

---

## RECORD Phase (delegated to `assistant`, runs every iter)
...
**No writes to memory** under any verdict. All session staging waits for Wrap-up to promote to `features/{feature-name}/...` after the workflow completes.
...
After `RECORD` (which always runs), the manager decides based on the reconciled verdict:

| Verdict | Action |
|---|---|
| `PASS` | Exit the loop; advance to Planning Loop. `outputs/` files + `staging/` artifacts are ready for Wrap-up's memory promotion |
...
├── transcripts/                       ← single session-root surface; {role}-{agentId}.jsonl per agent, all loops (RECORD copies in)
└── 1-ideation/
    ├── outputs/             ← PASS-iter output files (free filenames + mandatory frontmatter; assistant, RECORD, PASS only)
    ├── working/
...
**No writes to memory** during Ideation. The `features/{feature-name}/...` tree is created and populated by Wrap-up using `staging/` as the source — see [`wrap-up/SKILL.md`](../../wrap-up/SKILL.md).
