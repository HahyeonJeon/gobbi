...
| Key | Shape |
|---|---|
| `workflow.{step}` | Per step (same productive names as `state.json` / `settings.json`). `session.json` is schema 4; `state.json` / `settings.json` are schema 2. Configuration carries `startedAt` / `finishedAt` only; steps 2-5 add `iter` (final loop count), append-only `iterations[]` RECORD evidence, and `verdict` (`pass` \| `fail` \| `skipped`). |
| `workflow.{loop}.integration` | <ul><li>Per-step **value telemetry** — one object per productive loop (`ideation` / `planning` / `execution` / `wrap-up`); absent on `configuration` and `chat`. Seeded all-`0` by the template.</li><li>Four counts: `changing_rows`, `kept_own_rows`, `total_rows`, `escalated_rows` — derived from the loop's dual-system integration log (`reconciliation-iter{n}.md`) by the count rule below.</li><li>Measures how much the Codex proposal moved the canonical artifact.</li><li>`execution` additionally carries `tasks: []`; loop totals are the sum over task counts.</li></ul> |
| `workflow.chat.tasks[]` | <ul><li>Chat sessions only (`settings.mode == "chat"`; empty for Auto).</li><li>One entry per task slice: `taskNo`, `slug`, `startedAt`, `finishedAt`.</li><li>Per-loop sub-records `ideation` / `planning` / `execution` — same `{state, verdict, iter, maxIterations, phase, iterations[]}` shape as `workflow.{step}`.</li><li>`taskRecord: { path, writtenAt }`.</li><li>Present in both `state.json` (live projection) and `session.json` (final archive).</li></ul> |
...
| Each step transition / loop close / step exit | manager | `workflow.{step}.startedAt` / `finishedAt`; `iter` (steps 2-5); `verdict` (steps 2-5). For Chat: the matching `workflow.chat.tasks[]` sub-records. |
