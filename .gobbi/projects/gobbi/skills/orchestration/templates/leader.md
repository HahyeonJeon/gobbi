...
3. Skills:
   ...
   - Domain skills — list each `skills/{x}/SKILL.md` WITH its `skills/{x}/mistakes.md` companion on the next line (the per-skill mistakes companion path; subagents have no Skill tool, so an unlisted companion never loads): <<list — e.g., `.gobbi/projects/<<project-name>>/skills/ideation/SKILL.md`, `.../research/SKILL.md`. For gobbi planning, load BOTH the generic planning SOP `.gobbi/projects/<<project-name>>/skills/planning/SKILL.md` (planning craft) and the gobbi planning workflow doc `.gobbi/projects/<<project-name>>/skills/orchestration/workflow/planning.md` (loop mechanics — the same doc the Phase-doc line above resolves when `<<phase>>` is `planning`)>>
