...
- Does the skill add domain-specific best-practice knowledge that gobbi's `best-practice` does not already provide?
...
Stance skills are thin structurally — they govern thinking, not tool usage or file layout. The structural checks that apply are the universal ones from `claude`.
...
- Does the file avoid `claude` anti-patterns: no code examples, no BAD/GOOD comparisons, no step-by-step recipes?

### Integration

- Does the skill supplement gobbi's `best-practice` without contradicting its core principles (evidence over opinion, context-dependent, proven patterns)?
- Can the orchestrator load both gobbi's `best-practice` and this project skill together without conflicting instructions?
...
- `[semantic]` No duplication with gobbi's `best-practice` core principles (evidence over opinion, context-dependent, proven patterns)
...
- `[semantic]` Skill supplements gobbi's `best-practice` without contradicting it
