...
- `packages/cli/src/cli.ts:194` — `COMMANDS_BY_NAME as const satisfies Record<CommandName, CommandDef>`. `CommandName` is derived via `typeof COMMAND_ORDER[number]` at `cli.ts:95` from a `readonly` tuple at `cli.ts:85-93`. The two halves cross-check: add to the tuple and miss the map, or add to the map and miss the tuple — either way, `tsc` fails. The tuple controls `--help` ordering; the map supplies the dispatcher. One source of truth for the key set, enforced at two sites.
...
The codebase has roughly a dozen `assertNever` call sites (run `rg -n 'assertNever' packages/cli/src/` for the current list). Each sub-reducer in `packages/cli/src/workflow/reducer.ts` ends with one (`reducer.ts:244`, `:291`, `:339`, `:447`, `:503`, `:599`, `:648`); `packages/cli/src/specs/errors.ts:214-218` defines a module-local copy; `packages/cli/src/commands/workflow/guard.ts:384` and `packages/cli/src/commands/workflow/transition.ts:403` each have their own. The pattern is local to the module, small, and duplicated — this is deliberate. A central helper would introduce a cross-layer dependency for a four-line function; locality costs nothing and keeps layers clean.
...
`packages/cli/src/workflow/store.ts:220-248` declares `ReadStore` (nine pure-query methods) and `WriteStore extends ReadStore` (adds `append`, `transaction`, `close`). `EventStore` implements the wider interface; call sites declare the narrowest they need. `workflow/engine.ts`, `commands/session.ts`, `commands/workflow/stop.ts`, and `commands/workflow/capture-subagent.ts` all target `ReadStore` where they only query — the typechecker rejects `.append()` calls against those parameters.
...
A recurring shape: declare a `FooInput` interface (the on-disk or partially-populated form, with optional fields and nullable values) and a `Foo` interface (the fully hydrated form, no optionals where defaults exist, no nulls where the loader fills). Callers of the loader receive `Foo`; they never see `FooInput`. `packages/cli/src/lib/project-config.ts` defines `ProjectConfigInput` / `ProjectConfig` as the canonical example — the input admits every legal partial shape; the hydrated type is what downstream code reasons over.
