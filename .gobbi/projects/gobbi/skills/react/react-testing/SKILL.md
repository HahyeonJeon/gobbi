...
- Evidence: For each test defect, record its classification, root cause, test-local repair, and results from
  rerunning the narrow test, affected suite, and required wider checks. For each product defect, record its
  classification, product failure evidence, and the receiving skill, `react-development`; do not claim a
  test-local repair. For each environment gap, unsupported claim, or unresolved flake, record its
  classification, exact gap or reason for stopping, and matching terminal status without a test-local repair
  claim.
- Recovery: Keep each environment gap, unsupported claim, or unresolved flake visible with its exact gap or
  reason for stopping and matching terminal status; do not claim a repair. After two or three failed repairs,
  stop and reassess the design or request missing context; do not loop on retries or suppress the signal.
