...
The leader's role spans **both** DISCUSSION and WORK — same shape as Ideation. The assistant owns MEMORIZATION (loaded via [`memorization/SKILL.md`](../memorization/SKILL.md)). The evaluator owns EVALUATION (loaded via [`evaluation/SKILL.md`](../evaluation/SKILL.md)).
...
2. `git -C "$worktreePath" commit -m "chore(skills): promote {slug} generated by preparation iter{n}"` — with the required trailer in the commit body: `AI-Provenance-Record: gobbi://session/{session-id}/task/preparation-promote-now-iter{n}` per `git/conventions.md:118`.
