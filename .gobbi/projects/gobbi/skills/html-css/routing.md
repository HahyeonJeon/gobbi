...
| Single | Choose reduced, interrupted, and cancelled transition mechanics after motion is approved. | `html-css-motion` | None | Motion owns declarative mechanics; the interface owner still decides whether motion is warranted. | One match: Motion. |
| Multi | Design a responsive form system. | `html-css-design`, `html-css-semantics`, `html-css-conventions` | Interface owner supplies approved intent. | Design coordinates; Semantics chooses meaning; Conventions chooses binding presentation defaults. | Load all three with one owner per claim. |
...
| Non-match | Choose the product visual identity or decide whether motion is warranted. | None | `web-interface` or `desktop-interface` | HTML/CSS children do not own product direction. | Route to the named interface owner only. |
