...
Gobbi already provides orchestration (`gobbi-agent`), evaluation (`skills-evaluator`, `agent-evaluator`, `project-evaluator`), ideation (`pi`), research (`researcher`), and execution (`executor`). A project agent that duplicates these roles adds routing ambiguity without adding value. Project agents must bring domain knowledge that gobbi agents lack — a security reviewer who knows the project's auth stack, not a generic security reviewer.
...
- Is this agent distinct from gobbi's built-in agents? Does it fill a role that `gobbi-agent`, `pi`, `researcher`, `executor`, or the evaluator agents do not cover?
...
- Does the definition follow `claude` writing principles — principles over procedures, constraints over templates, codebase over examples?
...
- `[structural]` Agent filename uses hyphen word separators and no underscores; `name` frontmatter matches filename
