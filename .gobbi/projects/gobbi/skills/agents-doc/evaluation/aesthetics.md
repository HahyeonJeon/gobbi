...
The `name` frontmatter field must match the agent's filename exactly (without the `.md` extension). Agent names use hyphen word separators and no underscores. Check: name matches filename, multi-word names use hyphens not camelCase.

Common violations: name that doesn't match the filename; multi-word name using camelCase instead of hyphens; underscores used as word separators.
