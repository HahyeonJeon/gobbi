...
- **Then:** `ideation.tmp.md` remains available through evaluation but outside the returned candidate; each supporting draft includes its conditional supersession notice and enough context to stand alone; integration groups temporary records by topic, consolidates repeated discussion rounds into one current decision, merges duplicates, connects dependencies, propagates parent consequences, routes contradictions or unresolved choices back through discussion, and writes the coherent decision set back to `ideation.tmp.md` without creating `ideation.md`; integrated design review then exposes cross-topic gaps or contradictions, routes every material new or reopened choice through additional discussion and temporary reintegration, develops and records the coherent design in the temporary file, and only then creates the evolving `ideation.md`; every material decision records its complete decision tuple; changes are disclosed; and no ordered implementation tasks or implementation diff appears.
- **Failure oracle:** the temporary record is absent before evaluation, Step 3.3 creates or updates `ideation.md`, integration copies the temporary record chronologically instead of synthesizing it, integrated decisions are not written back to `ideation.tmp.md`, duplicate or stale decisions survive, a contradiction or unresolved choice is resolved by assumption, integrated review discovers a material choice that bypasses discussion and reintegration, `ideation.md` is created before the integrated design is coherent, the final document omits a material temporary discussion or decision, the final document says “see requirements,” a topic asks an open-ended question without presenting options and a recommendation, the hierarchy advances past an unresolved ancestor, any material decision field is missing, an ordered implementation task list or implementation diff appears, one returned file is absent, `ideation.tmp.md` is returned, the contingent-authority statement is missing, or final approval or authority is claimed before evaluation passes.
- **Evidence tuple:** group the pre-integration temporary records by topic, compare every discussion round with the decision set written back to `ideation.tmp.md`, inspect duplicate handling, dependency and consequence propagation, conflict routing, integrated-design review, additional discussions, reintegration, and the later temporary-to-final transition, then perform three isolated cold reads and the output-boundary sweeps; complete parent-first temporary synthesis, iterative design refinement, standalone final meaning, approval readiness, and a clean three-document output boundary confirm the case.
