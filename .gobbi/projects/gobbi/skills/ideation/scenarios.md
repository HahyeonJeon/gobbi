...
- **Then:** `ideation.tmp.md` remains available through evaluation but outside the returned candidate; each supporting draft includes its conditional supersession notice and enough context to stand alone; integration groups temporary records by topic, consolidates repeated discussion rounds into one current decision, merges duplicates, connects dependencies, propagates parent consequences, and routes contradictions or unresolved choices back through discussion before writing the evolving `ideation.md`; integrated design review exposes cross-topic gaps or contradictions, routes every material new or reopened choice through additional discussion and temporary recording, and reintegrates the result before design development continues; every material decision records its complete decision tuple; changes are disclosed; and no ordered implementation tasks or implementation diff appears.
- **Failure oracle:** the temporary record is absent before evaluation, integration copies the temporary record chronologically instead of synthesizing it, duplicate or stale decisions survive, a contradiction or unresolved choice is resolved by assumption, the initial integration is treated as final, integrated review discovers a material choice that bypasses discussion and reintegration, the final document omits a material temporary discussion or decision, the final document says “see requirements,” a topic asks an open-ended question without presenting options and a recommendation, the hierarchy advances past an unresolved ancestor, any material decision field is missing, an ordered implementation task list or implementation diff appears, one returned file is absent, `ideation.tmp.md` is returned, the contingent-authority statement is missing, or final approval or authority is claimed before evaluation passes.
- **Evidence tuple:** group `ideation.tmp.md` by topic and compare every discussion round with the consolidated decision set and each integrated draft, inspect duplicate handling, dependency and consequence propagation, conflict routing, integrated-design review, and every additional discussion and reintegration, then perform three isolated cold reads and the output-boundary sweeps; complete parent-first decision synthesis, iterative integration, standalone meaning, approval readiness, and a clean three-document output boundary confirm the case.
...
- **Obligation:** Ideation consolidates temporary discussion rounds into a coherent parent-first decision set, routes contradictions and unresolved choices back to the user, treats each integration as an evolving design input, routes integration-created choices through additional discussion and reintegration, deletes the temporary record only after approval and verification, and presents three independently readable documents whose authority remains contingent until approval and whose output contains no ordered implementation tasks or implementation diff.
