...
2. **Else scan the type's fixed priority-ordered tag→area map; the FIRST area whose any mapped tag is present wins** (mistakes uses the curated trap-class map; spine types use the spine map — both in §1.5). First, **normalize a feature-dir input to a spine area** (`git-workflow → git`, `workflow → workflow`) before matching.
3. **Else `_shared/`** — the no-match bucket. Never invent a new area to avoid `_shared`.

The allowlist is closed (§1.5): `mistakes` → `verification · rename-sweep · tooling · git · codex · docs-sync · memory · _shared`; the spine types → `memory · git · workflow · wrap-up · evaluation · codex · process · _shared`. An area outside the type's allowlist is a validator failure. Record the resolved area in `working/promotion-manifest.md`.
