...
**Inputs**
- All prior loops' staging trees: `sessions/{date}-{session-id}/{1-ideation,2-preparation,3-planning,4-execution}/staging/`, plus `interview/staging/` when an interview ran this session, plus the Chat-mode per-slice staging `chat/tasks/*/{N}-{loop}/staging/` (and `chat/tasks/*/4-execution/task-*/staging/`) when the session ran in Chat mode
...
- **MUST account for every staging file** — `promotion-manifest.md` has 1 entry per staging file across all prior top-level loops, Chat slice loops, nested Chat execution tasks, and `interview/staging/` when present (promoted / backlogged / dropped with reason).
