...
| `sessions/.../{N}-{loop}/staging/decisions/{slug}.md` with frontmatter `mistake-candidate: true` | `features/{feature-name}/mistakes/{area}/{slug}.md` (feature-scope) OR `mistakes/{area}/{slug}.md` (project-scope) | Return `NEEDS_CONTEXT`; manager confirms scope through the active runtime's user-decision primitive |
...
This is the **single place** the area is resolved at promotion (WORK Step 4 step (d)). For every by-area staging file, Wrap-up resolves the destination `{area}/` segment via the **TOTAL deterministic area-selection rule** in [`memory/rules.md` § 1.5](../memory/rules.md#15-area-namespace-the-second-category-axis-under-each-type) — the SAME rule and SAME priority order the validator's area allowlist enforces, so the written path and the validator's path-derived check agree by construction. The rule is total (every record resolves to exactly one area) and deterministic (priority-ordered first-match):
...
2. **Else scan the type's fixed priority-ordered tag→area map; the FIRST area whose any mapped tag is present wins** (mistakes uses the curated trap-class map; spine types use the spine map — both declared in [`memory-vocabulary.json`](../../memory-vocabulary.json) `.tagAreaMap`, which Wrap-up reads for area RESOLUTION following this prose spec; the validator does NOT read `.tagAreaMap` — it enforces the resolved area against `.effective.areas.*`; the priority semantics are in §1.5). First, **normalize a feature-dir input to a spine area** per the config's `.tagAreaMap.featureDirNormalization` before matching.
3. **Else `_shared/`** — the no-match bucket. Never invent a new area to avoid `_shared`.

The allowlist is closed (§1.5): the project declares it in [`memory-vocabulary.json`](../../memory-vocabulary.json) — `.effective.areas.mistakes` (the curated trap-class set) and `.effective.areas.spine` (the shared spine), the SAME arrays the validator enforces. An area outside the type's allowlist is a validator failure. Record the resolved area in `working/promotion-manifest.md`.
