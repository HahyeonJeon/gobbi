...
- **WORK is the assistant's domain**, and Wrap-up's WORK is the **sole writer to memory for cross-loop session artifacts** — subject to two bounded pre-Wrap-up exceptions. Every other loop's RECORD stages typed-finding artifacts into session record only — Wrap-up reads those staging directories and promotes them. The two bounded exceptions are (1) `startup`-close promotion, which writes the startup skill's user-approved baseline to memory before any productive loop runs — Wrap-up EXCLUDES `startup/` from its promotion inventory (§ Promotion-inventory rule), so it never re-promotes what startup already promoted; and (2) Preparation's `generate-now` skills, which the manager promotes before Planning starts so in-session consumers can load them.
...
| **Session record — all prior loops** | `sessions/{date}-{session-id}/{1-ideation,2-preparation,3-planning,4-execution}/{outputs,staging,evaluation,working}/` — plus the Chat-mode per-slice tree `chat/tasks/*/{N}-{loop}/{outputs,staging,evaluation,working}/` and `chat/tasks/*/4-execution/task-*/staging/` in Chat-mode sessions | **READ-ONLY** — required inputs: every prior loop's artifacts (what shipped), staging (what to promote), evaluation outputs (cross-loop closure audit), discussion logs |
...
Ideation / Planning / Execution RECORD write **only** to session record under `sessions/{date}-{session-id}/{N}-{loop}/`. Wrap-up reads accumulated `staging/` directories across all loops and promotes to `.gobbi/projects/{project-name}/...`. No phase other than the two bounded pre-Wrap-up exceptions — `startup`-close promotion and Preparation's `generate-now` skills — and Wrap-up writes to memory.

**Narrow exception — Preparation's generated skills:** when Preparation's EVALUATION verdicts `PASS` and the loop produced `generate-now` project-specific skills, the manager promotes those skills from `sessions/{date}-{session-id}/2-preparation/staging/skills/{slug}/SKILL.md` to `.gobbi/projects/{project-name}/skills/{slug}/SKILL.md` **before Planning starts**. This is one of two bounded pre-Wrap-up memory writes in the workflow — the other is `startup`-close promotion (§ Promotion-inventory rule; Wrap-up EXCLUDES `startup/`, so its self-promoted baseline is never re-promoted). Its scope is strictly bounded to skill files generated by Preparation's `generate-now` decision; all other Preparation staging waits for Wrap-up. Wrap-up's memory write authority is otherwise exclusive and covers all remaining session artifacts. See `preparation/SKILL.md` § Core Principles for the rationale and mechanics.

> **Mistake promotion — two homes, routed by ownership.**

Beyond promoting staging → memory, the Wrap-up assistant routes each staged mistake-candidate to one of two homes (Always-Ask, confirmed with the user during Wrap-up DISCUSSION): a **skill-owned** trap becomes a `## ` section in `skills/{skill}/mistakes.md`, loaded in that skill's context via the delegation Load-Directives companion path; a **cross-cutting / no-owner** trap stays in the project `mistakes/` tier at `mistakes/{area}/{slug}.md`, loaded at session start by [`mistake/SKILL.md`](../mistake/SKILL.md) P1. The assistant proposes skill-owner-vs-project at promotion; the user confirms. No CLI command is required — the Wrap-up assistant is the sole documented promotion mechanism. Promotion does not cause context reload.
...
Every file under `sessions/.../{N}-{loop}/staging/` across every prior top-level loop, every Chat slice loop, and every nested Chat execution task in this session must be accounted for. Three outcomes are valid:
...
> **Promotion-inventory rule — inventory `staging/` ONLY (with one named exclusion: `startup/`).**
...
- **Chat-mode per-slice `staging/` (Chat-mode sessions only)** — when the session ran in Chat mode, each per-task slice runs the unmodified base RECORD ([`orchestration/chat-mode.md` §4](../orchestration/chat-mode.md)) and writes its typed findings under `chat/tasks/*/{N}-{loop}/staging/` (and `chat/tasks/*/4-execution/task-*/staging/` for execution sub-tasks). Wrap-up MUST enumerate these globs so Chat-mode typed findings reach memory. A non-Chat session has no `chat/tasks/` tree, so this source is simply absent there — non-Chat Wrap-up still promotes from the workflow-loop `staging/` (`1-ideation`..`4-execution`) only.

> **`startup/` is EXCLUDED — a named exclusion despite carrying a `staging/`.** The `startup` bootstrap surface (`sessions/{date}-{session-id}/startup/staging/`) is **NOT** a Wrap-up promotion source, even though it holds a `staging/`. Unlike a workflow loop, `startup` promotes its OWN staging at startup-close — before any productive loop runs — writing its user-approved records directly to memory and recording the promoted paths + a completion marker in `startup/outputs/startup-summary.md` (see [`startup/recording.md` § 9](../startup/recording.md)). By the time any same-session Wrap-up runs, `startup/staging/` is already durable. Enumerating it here would **re-promote** an already-promoted surface — a double-promotion. So Wrap-up EXCLUDES the entire `startup/` tree.

**F-P2 — do not over-narrow, and do not re-include `startup/`.** The never-promote exclusion targets `transcripts/`, `working/`, `evaluation/`, and `outputs/` — it does **not** exclude the Chat-mode per-slice `staging/`. The rule is "inventory `staging/` only", not "inventory workflow-loop `staging/` only": narrowing it to drop the Chat-mode per-slice `staging/` would lose promotions and is wrong. The ONE staging-bearing dir that IS excluded is `startup/` — because startup already promoted it at startup-close, so re-including it would **double-promote**. See [`record/record-map.md` § Wrap-up promotion-inventory rule](../record/record-map.md) for the authoritative statement.
...
**Inputs**
- All prior loops' staging trees: `sessions/{date}-{session-id}/{1-ideation,2-preparation,3-planning,4-execution}/staging/`, plus the Chat-mode per-slice staging `chat/tasks/*/{N}-{loop}/staging/` (and `chat/tasks/*/4-execution/task-*/staging/`) when the session ran in Chat mode. The `startup/` surface is EXCLUDED — startup promotes its own staging at startup-close (see § Promotion-inventory rule), so Wrap-up never re-promotes it
...
| 2 | **1 — validation** | **Enumerate all staging across all loops** | For each loop directory in `sessions/{date}-{session-id}/{1-ideation,2-preparation,3-planning,4-execution}/` **plus each Chat-mode per-slice dir `chat/tasks/*/{N}-{loop}/` and `chat/tasks/*/4-execution/task-*/`** (Chat-mode sessions only), recursively list `staging/` — and **only** `staging/`; never `transcripts/`, `working/`, `evaluation/`, or `outputs/`, and never the `startup/` surface (startup owns its own startup-close promotion — per § Promotion-inventory rule). Build a master inventory at `sessions/{date}-{session-id}/5-wrap-up/working/staging-inventory.md` — every staging file path, sized + frontmatter-extracted. **Step 2.5 runs immediately after this step** — see `### Step 2.5` below for the prior-loop RECORD compliance scan that completes stage 1 and must finish (gap → auto-backfill or `NEEDS_CONTEXT`) before stage 2 begins |
...
- **Two-phase freeze boundary.** The Codex proposal is **frozen** before the assistant integrates it; the canonical promotion outputs (the promotion manifest + handoff) are **frozen** before EVALUATION (stage-3 memory validation) spawns. The assistant integrates against the frozen proposal — it never races a still-writing Codex run — and the canonical outputs do not change under the evaluator. Derived from [`evaluation/mistakes.md#freeze-producer-artifact-before-evaluating`](../evaluation/mistakes.md#freeze-producer-artifact-before-evaluating).
...
Stage 3 is non-skippable (D13), so this green-check runs on **EVERY** wrap-up — whether or not the dormant Stage-2c compaction ran. Before the verdict can advance to `PASS`, the manager re-runs **EVERY** standing project guard over the **post-promotion** tree, not only the frontmatter validator. A promoted mistake / journal / plan frequently DOCUMENTS the very vocabulary or pattern a guard scans for, so a promotion can add a legitimate carrier that an allowlist derived before the promotion will not recognize. Derived from [`mistakes.md#wrap-up-green-check-must-rerun-standing-guards-post-promotion`](mistakes.md#wrap-up-green-check-must-rerun-standing-guards-post-promotion).
...
- **MUST be the sole writer to memory for cross-loop session artifacts** — no other loop writes to `.gobbi/projects/{project-name}/{features,mistakes,rules,design,notes,backlogs,references,decisions,plans,reviews,reports,learnings,archive}/`. Two bounded pre-Wrap-up exceptions: (1) `startup`-close promotion writes the startup skill's user-approved baseline to memory before any productive loop — Wrap-up EXCLUDES `startup/` from its inventory and never re-promotes it; (2) Preparation-generated skills at `.gobbi/projects/{project-name}/skills/{slug}/SKILL.md` are promoted by the manager before Planning starts (see `preparation/SKILL.md` § Core Principles). At Wrap-up, verify the destination is present and record in `promotion-manifest.md`; do not re-promote unless the destination file is missing.
- **MUST account for every staging file** — `promotion-manifest.md` has 1 entry per staging file across all prior top-level loops, Chat slice loops, and nested Chat execution tasks (promoted / backlogged / dropped with reason). The `startup/` surface is EXCLUDED — startup promotes its own staging at startup-close, so Wrap-up never re-promotes it.
