...
| `sessions/.../{N}-{loop}/staging/decisions/{slug}.md` with frontmatter `mistake-candidate: true` | `features/{feature-name}/mistakes/{slug}.md` (feature-scope) OR `mistakes/{slug}.md` (project-scope) | Return `NEEDS_CONTEXT`; manager confirms scope via AskUserQuestion |
...
| `sessions/.../{N}-{loop}/staging/design/{slug}.md` | `features/{feature-name}/design/{slug}.md` (default) OR `design/{slug}.md` (project-wide; rare) | If project-wide, return `NEEDS_CONTEXT`; manager confirms via AskUserQuestion |
...
| `sessions/{date}-{session-id}/5-wrap-up/working/discussion-log.md` | manager (DISCUSSION) | appended per AskUserQuestion exchange |
