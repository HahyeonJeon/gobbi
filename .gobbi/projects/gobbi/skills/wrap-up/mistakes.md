...
## Wrap-Up Additions Must Enter Typed Staging Before Promotion

`priority: high` · `domain: process` · `added: 2026-07-16` · `status: active` · `tags: [process, verification]`

**What happened** — The Wrap-up loop itself surfaced a new promotable item — a deferred-decision backlog entry, a session-surfaced rule or mistake candidate — and HAND-CREATED the file directly at its memory destination (e.g. `backlogs/workflow/{slug}.md`) or under a non-staging dir after inventory freeze, instead of routing it through typed staging + the promotion manifest. The addition bypassed typed routing, source accounting, collision allocation, and whole-batch preflight — so a re-run cannot reconcile it and the audit trail has a record no manifest explains.
**Why it happens** — Wrap-up is the memory writer, so a discovery made during Wrap-up is mistaken for permission to write a new record immediately. Writer authority does not remove the staging + manifest contracts that make the write auditable and atomic; the promotion pipeline (inventory → manifest → apply) is idempotent ONLY because every promoted record enters through typed staging first.
**How to detect** — A promotion-manifest source row points into a non-staging dir (`sessions/{date}-{session-id}/5-wrap-up/{working,evaluation,outputs}/`) for a newly discovered promotable item; a durable record has no typed `staging/{type}/{slug}.md` source; or a new record appears after manifest freeze. The only permitted non-staging sources are the procedure's explicitly enumerated direct candidates.
**Correct approach** — Put every newly discovered promotable addition into the matching typed `5-wrap-up/staging/{type}/{slug}.md` path — at this session's RECORD — then restart Stage 1 so it is inventoried, routed, validated, collision-planned, and frozen with the whole batch, one source identity and one manifest row, idempotent on re-run. Do NOT widen the non-staging working area into a general source. The two Wrap-up-authored non-staging sources the current contract defines (a user-confirmed rule candidate + the per-session journal) are the only named exceptions, and each is rendered as ONE deterministic direct candidate before manifest freeze.
