...
- For the **Execution** loop, RECORD additionally appends a per-task element to `workflow.execution.integration.tasks[]`: `{ taskNo, slug, changing_rows, kept_own_rows, total_rows, escalated_rows }` (D4.3 per-task value). The append is idempotent — keyed by `taskNo`, re-runs overwrite rather than duplicate.
