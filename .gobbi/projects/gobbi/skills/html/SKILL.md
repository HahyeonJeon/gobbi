...
Electron renderer work also routes through [`electron`](../electron/SKILL.md), whose root selects every applicable Electron child. A complete installed Electron and TypeScript outcome, observable installed-platform behavior, or desktop release judgment also routes through [`desktop`](../desktop/SKILL.md). HTML children own only the emitted markup.
