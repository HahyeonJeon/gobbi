...
- **GB-3 — Load a conditional owner before its action.** Load `codex` when the active runtime is native Codex or an operation uses a Codex peer surface. Before authoring a specialist brief in a workflow session, load the indexed [`workflow`](../workflow/SKILL.md) owner and apply its Step 1.3 additions to the floor-loaded Delegation skill. The floor already covers `discussion` and `git`, so they are not conditionally loaded here.
...
Consult the skill map and load only the indexed owner whose trigger applies. Read [`../codex/SKILL.md`](../codex/SKILL.md) when the active runtime is Codex or any operation uses a Codex peer. For a workflow brief, use the floor-loaded [Delegation](../delegation/SKILL.md) skill with [`Workflow` Step 1.3](../workflow/SKILL.md#13-build-and-accept-specialist-assignments). Load a language skill when the task enters its domain. These sources define their own mechanics; Gobbi does not copy them.
