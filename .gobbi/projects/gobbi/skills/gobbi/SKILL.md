...
**Designing scenarios and checklists is a reference skill, not a table row (this session).** `scenario` teaches how to design and write a scenario set (the Category → Family → Case → Check taxonomy, author-declared primary categories, the coverage-role and n-ary inseparability record, and the failability teeth); `checklist` teaches how to design and write a checklist (the three modes, the five orthogonal item dimensions, the closed resolution state machine, and the two-gate coverage-vs-acceptance rule). Both are the shared basis of **design and evaluation** — a design phase authors a scenario set / checklist to frame obligations, and an evaluator reuses the same artifacts to frame coverage. They are on-demand authoring/reference skills mirrored to both runtimes (`.claude/skills/` + `.agents/skills/`). Until the deferred per-loop pointer wiring and conformance sweep land (they are unwired authoring references for now), they carry NO Loop / Cross-cutting / Supporting Skill-Map row; this paragraph is their home.
