...
- **Promotion at Wrap-up (Always-Ask routing):** The Wrap-up assistant promotes each staged candidate to one of two homes. A **skill-owned** trap becomes a `## ` section in `skills/{skill}/mistakes.md`, loaded in that skill's context via the delegation Load-Directives companion path. A **cross-cutting / no-owner** trap stays in the project memory tier (`.gobbi/projects/{project-name}/mistakes/{area}/{slug}.md`), loaded at session start by `mistake/SKILL.md` P1. No CLI command — the Wrap-up assistant is the sole promotion mechanism. Promotion does not cause context reload.
