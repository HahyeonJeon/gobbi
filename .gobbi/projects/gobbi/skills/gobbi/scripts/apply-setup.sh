...
# One ledger row per target, in write order. A negative result is an asserted row, never an omission.
record() {
  case "$2" in
...
# A refusal, not a skip: the run ends, nothing further is created, and the exit status is nonzero.
refuse() {
  printf 'REFUSED %s: %s\n' "$1" "$2" >&2
  printf 'probe: %s\n' "$3" >&2
  printf 'created: 0 filesystem objects\n' >&2
...
# nothing at all. The components checked are relative to the project root: a consumer project that lives
# under a directory named skills is not a refusal, while any Gobbi-owned skills path is.
guard_target() {
  local relative="$1"
  ...
  if [[ "$absolute" != "$project_root"/* ]]; then
    refuse refusal-1 "computed target $absolute is outside $project_root" \
      'git rev-parse --show-toplevel against the computed target'
  fi
...
ensure_directory() {
  ...
  if ! parent_is_real_directory "$relative"; then
    record "$relative/" stopped "S4 the parent is not a real directory"
    return 1
  fi
  if [[ -L "$absolute" ]]; then
