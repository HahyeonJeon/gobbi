...
Each intent map section cross-references the domain skill that owns the workflow context. `gobbi-cli` tells you the command exists and what arguments it takes. `note`, `claude`, and `notification` tell you the full workflow around that command.
...
Cross-reference: `orchestration`, `note` for workflow setup context. `gobbi` skill for session setup questions. `gobbi-config/README.md` for the full three-level cascade.
...
Cross-reference: `note` for the full note-writing workflow and subtask JSON format.
...
Cross-reference: `claude` for the documentation standard and writing guidelines.
...
Cross-reference: `notification` for channel configuration and credentials setup.
...
- This skill documents what commands exist and when to use them; domain skills (`claude`, `note`, `notification`) teach the full workflow context
