...
Cross-reference: `note` for note directory structure, subtask JSON format, and when to write at each workflow step.
...
Cross-reference: `skills-doc` for skill structure requirements, `agents-doc` for agent definition requirements, `gotcha` for gotcha entry format.
...
Cross-reference: `notification` for channel configuration, credentials setup in `.claude/.env`, and hook integration.
