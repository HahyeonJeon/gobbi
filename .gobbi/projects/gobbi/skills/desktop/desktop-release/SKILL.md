...
`desktop-electron` owns release mechanisms and current platform details. `desktop-delivery` owns ordered work for a complete outcome; this skill owns only the valid choice space, defaults, exceptions, and authority boundaries for release decisions.
