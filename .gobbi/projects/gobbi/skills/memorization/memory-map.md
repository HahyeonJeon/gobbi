...
Project-specific skill / agent overrides live under runtime static-knowledge paths: `.claude/skills/{skill-name}/` and `.claude/agents/{agent-name}.md` in Claude Code, `.agents/skills/{skill-name}/` and `.codex/agents/{agent-name}.toml` in Codex. These are static-knowledge paths, **not project memory** in this skill's sense. The memory-map only covers paths under `.gobbi/projects/{project-name}/`; runtime overrides are authored manually by the maintainer and are out of MEMORIZATION's scope.
