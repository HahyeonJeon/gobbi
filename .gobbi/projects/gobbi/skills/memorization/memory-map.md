...
| `{N}-{loop}/working/discussion-log.md` | Manager-captured AskUserQuestion exchanges, appended one section per exchange. Preserved across iters | manager (DISCUSSION live) | every AskUserQuestion exchange | — |
...
| `{N}-{loop}/staging/discussions/{slug}.md` | Substantive AskUserQuestion topic from discussion-log | assistant (MEMORIZATION) | `PASS` only | [`templates/discussions.md`](templates/discussions.md) |
