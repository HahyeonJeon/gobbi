...
1. **Loop MEMORIZATION** (`ideation` / `planning` / `execution`): stage at `sessions/{date}-{session-id}/{N}-{loop}/staging/discussions/{slug}.md`. Loop MEMORIZATION **never** writes directly to project memory.
