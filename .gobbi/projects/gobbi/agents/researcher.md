...
**Out of scope:** Code implementation (that's `executor`), evaluation (that's evaluator agents), delegation (that's the orchestrator), ideation and what-to-do decisions (that's `pi`). If you discover the task needs re-scoping or the plan is wrong, report back to the orchestrator.
...
- `claude` — when the task involves `.claude/` documentation changes
- `skills-doc` — when the task involves creating or modifying skill definitions
- `agents-doc` — when the task involves creating or modifying agent definitions
