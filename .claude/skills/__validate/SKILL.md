...
Two categories of validation tools exist: `gobbi validate` CLI commands for Markdown-based files and the `gobbi docs validate` CLI command for JSON templates.

**CLI commands** are invoked via `gobbi validate`. Each takes a file path as its only argument, reports results to stdout/stderr, and exits 0 on pass or 1 on failure.

| Command | Validates | Use when |
|---|---|---|
| `gobbi validate agent` | Agent definition `.md` files | After creating or modifying an agent in `agents/` |
| `gobbi validate skill` | Skill `SKILL.md` files | After creating or modifying a skill's SKILL.md |
| `gobbi validate lint` | Any `.md` skill file for anti-patterns | After writing any `.claude/` documentation |
| `gobbi validate gotcha` | Gotcha `.md` files | After recording a new gotcha entry |

Run from the repository root or provide absolute paths. Commands are self-contained with no external dependencies beyond standard bash utilities.
...
**gobbi validate agent** verifies the structural contract that the orchestrator depends on: YAML frontmatter with required fields, name format conventions, trigger language in the description, valid model values, and sufficient system prompt content.

**gobbi validate skill** verifies the structural contract that skill loading depends on: YAML frontmatter with required fields, line budget compliance, and navigation structure when child documents exist.

**gobbi validate lint** detects _claude anti-patterns that cause agents to mimic instead of think: code examples, BAD/GOOD comparison blocks, step-by-step recipes, and interface definitions. Uses heuristics to distinguish illustrative directory trees (acceptable) from code examples (not acceptable).

**gobbi validate gotcha** verifies the entry structure that makes gotchas useful: required sections (title, priority, what happened, user feedback, correct approach) and valid priority values.
...
- Validation commands are tools, not documentation — they check structure, not content quality
...
- Validation commands must not modify files — read-only validation only
