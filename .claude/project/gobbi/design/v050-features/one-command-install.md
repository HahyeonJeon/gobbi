...
The behavioral rule that governs every session — `rules/_gobbi-rule.md` — ships with the plugin and is refreshed whenever the plugin updates. There is no version drift between what the plugin ships and what a session loads. Projects never need to maintain their own copy of gobbi's rules, agents, or hooks; they inherit them through the plugin relationship.
