...
## Hard-for-agents helpers (name TBD)
...
**Image (name TBD)** — analyzes images (metadata extraction, resize), compares images side-by-side as comparison sheets. Agents working on visual tasks load the CLI rather than attempt image processing inline.

**Video (name TBD)** — analyzes video files and extracts frames. Frame extraction has format-specific edge cases; the CLI handles them.

**Web (name TBD)** — takes screenshots of web pages and captures images from URLs. Agents performing UI review or visual diffing use this rather than a bare browser invocation.

**Notify (name TBD)** — sends notifications via Slack, Telegram, and Desktop with channel-specific formatting. Agents fire notifications at workflow milestones without needing channel-specific API knowledge.
