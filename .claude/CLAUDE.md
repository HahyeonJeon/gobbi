...
wording may support a recommendation but never records the selection. After the mode, ask a privacy-warned
normalized session slug for Cowork or Workflow; General skips it. Then ask one session-wide
...
Cowork owns its own native TODO route, Git contract, evaluation policy, and session locations. Its explicit
closure applies `memory` directly, commits durable updates or proves none are needed, then checks evaluation
freshness. It never loads `wrap-up` or creates Workflow-formatted TODOs, phase receipts, RECORD-stage evidence,
a tracked handoff, or a Workflow Hand-off. Workflow uses the native TODO list as its active route; phase
receipts and committed evidence rebuild that route after a context boundary.
...
Immediately after mode selection, publish the complete fixed native TODO template through Claude Code task
controls before asking for the applicable slug or partner policy. Keep dynamic topic, task, subject, stage,
iteration, and closure values in evidence and assignments, not TODO titles.
...
preparation, launch, validation, and returned frozen content. In Claude Code the partner is Codex. Each mode
owner owns local participants, complete-round assembly, acceptance, and its evaluation commitment. Disabled
invokes no external runtime; enabled calls Partner for every applicable external run without another prompt.
