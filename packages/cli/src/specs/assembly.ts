...
function renderBlockContent(block: BlockContent): string {
  // `BlockContent.refs` (shared-block inlining) is A.8's responsibility —
  // the skill loader inlines those at spec-load time. A.4 treats `refs` as
  // already resolved by the time it sees the block.
